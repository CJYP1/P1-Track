...
class Component extends DCLogic {
  ...
  _delayBaseStats(lv,z){const q=this._delayQtyStats(lv,z),programme=this._p6ZoneDelayDays(lv,z),qd=q?q.days:null,vals=[programme,qd].filter(Number.isFinite),days=vals.length?Math.max.apply(null,vals):null;return {days,programme:Number.isFinite(programme)?Math.max(0,Math.round(programme)):null,quantity:Number.isFinite(qd)?Math.max(0,Math.round(qd)):null,quantityStats:q};}
  ...
  openDelayAdmin(){if(!this.rwsIsAdmin()){this.rwsDeny('Only admin can edit Delay days.');return;}const old=document.getElementById('__delayAdmin');if(old)old.remove();
    ...
    const body=rows.map((r,i)=>{let raw=Object.prototype.hasOwnProperty.call(map,r.key)?map[r.key]:'';if(raw&&typeof raw==='object')raw=raw.days!=null?raw.days:raw.delay_days;if(raw==null)raw='';if(raw!==''&&Number.isFinite(Number(raw)))raw=Math.max(0,Math.round(Number(raw)));const bs=this._delayBaseStats(r.lv,r.z),qs=bs.quantityStats,base=bs.days,eff=raw!==''?Number(raw):base,dv=eff==null?null:this._delayView(eff),parts=[];if(bs.programme!=null)parts.push('Project programme −'+bs.programme+' days');if(bs.quantity!=null)parts.push('Quantity indication −'+bs.quantity+' days');const auto=base==null?'—':`<b style="color:${this._delayView(base).c}">${this.esc(this._delayView(base).txt)}</b><small style="display:block;margin-top:3px">${this.esc(parts.join(' · '))}${qs?' · Actual '+this.esc(qs.progressPct)+'% of plan-to-date':''}</small>`;return `<tr data-q="${this.esc((r.lv+' '+r.label+' '+r.cat+' '+r.group).toLowerCase())}"><td>${this.esc(r.lv)}</td><td><b>${this.esc(r.label)}</b>${r.group?`<small>${this.esc(r.group)}</small>`:''}</td><td>${this.esc(cats[r.cat]||r.cat)}</td><td class="delay-base" data-days="${base==null?'':this.esc(String(base))}">${auto}</td><td><div class="delay-edit"><input class="delay-admin-in" type="number" min="0" step="1" data-key="${this.esc(r.key)}" value="${this.esc(String(raw))}" placeholder="Auto"><span>days</span><button type="button" class="delay-clear" title="Clear override and use project-based Auto">↺</button></div></td><td class="delay-result" style="color:${dv?dv.c:'#667085'}">${dv?this.esc(dv.txt):'—'}</td></tr>`;}).join('');
    const ov=document.createElement('div');ov.id='__delayAdmin';ov.innerHTML=`<div class="delay-admin-box"><div class="delay-admin-head"><div><b>Delay Table</b><span>Project programme delay + live cumulative quantities · no “ahead” result</span></div><button class="hbtn" id="__delayClose">Close ✕</button></div><div class="delay-admin-tools"><input id="__delayFind" placeholder="Find level or zone…"><span>${rows.length} zones</span></div><div class="delay-admin-scroll"><table><thead><tr><th>Level</th><th>Zone</th><th>Area</th><th>Project-based Auto</th><th>Override</th><th>Map display</th></tr></thead><tbody>${body}</tbody></table></div><div class="delay-admin-foot"><span>Auto keeps the existing project-programme delay as the baseline, then compares it with the live cumulative Plan/Actual indication and uses the more delayed result. Extra production first clears backlog; it never creates an “ahead” status.</span><button class="hbtn primary" id="__delaySave">Save Delay Table</button></div></div>`;
