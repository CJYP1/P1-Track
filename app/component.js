...
class Component extends DCLogic {
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0)),cell='padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right';return `<div style="overflow-x:auto"><table style="width:100%;min-width:610px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Plan</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Progress Applied</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Achieved</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':qty(r.plan,r.unit,false)}</td><td style="${cell};background:${r.applied>0?'#edf8f1':'transparent'}">${qty(r.applied,r.unit,r.applied>0)}<small style="display:block;color:#667085;font-size:7px">${r.completedEarlier?'Cumulative completed quantity':this.esc(mon)+' Actual '+this.esc(fmt(r.currentDone))+(r.earlierCredit>0?' + earlier completion '+this.esc(fmt(r.earlierCredit)):'')}</small></td><td style="${cell}"><b style="font-size:11px;color:${r.complete?'#167846':'#c8102e'}">${r.monthPct}%</b></td><td style="${cell}"><b style="color:${r.behind>0?'#c8102e':'#167846'}">${r.behind>0?this.esc(fmt(r.behind)):'—'}</b>${r.behind>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:105px">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':(r.complete?'<b style="color:#167846">Plan + previous backlog achieved</b>':`<b style="color:#c8102e">${this.esc(mon)} outstanding</b>`)}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
