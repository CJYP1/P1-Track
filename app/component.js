...
class Component extends DCLogic {
  ...
  addActCmt(lv,zmk,aid,text,to,meta){text=(text||'').trim();if(!text)return;to=String(to||'');if(to!=='PM'&&to!=='Planning'){this._toast&&this._toast('Please choose To PM or To Planning');return;}const report=!!(meta&&meta.report),allowed=this.rwsIsAdmin()||this.rwsCanComment(lv,zmk)||(report&&this.rwsScopeOk(lv,zmk));if(!allowed){this.rwsDeny('You cannot comment on this area.');return;}const s=(typeof rwsGetSession==='function'&&rwsGetSession())||{};const ts=new Date().toISOString();const k=lv+'||'+zmk+'||'+aid+'||'+ts+'_'+Math.random().toString(36).slice(2,6);const v={t:text,to,by:s.username||'user',ts,...(meta||{})};this._actCmt=this._actCmt||{};this._actCmt[k]=v;this.saveActCmt();if(typeof rwsSyncKV==='function')rwsSyncKV('act_cmt',k,v,lv,zmk);this._actRerender(this._selZone());}
  ...
  _cmtPanel(lv,zmk,aid,reportCat){if(this._cmtOpen!==lv+'||'+zmk+'||'+aid)return '';const all=this.actCmts(lv,zmk,aid);const admin=this.rwsIsAdmin();const canAdd=admin||this.rwsCanComment(lv,zmk)||(!!reportCat&&this.rwsScopeOk(lv,zmk));   // Report 评论也允许对应区域团队填写
    ...
    const row=(c,res)=>`<div class="cmtrow${res?' cmt-done':''}">${res?(admin?'<input type="checkbox" class="cmt-unresolve" data-k="'+this.esc(c.k)+'" checked title="Reopen (admin)">':'<input type="checkbox" checked disabled title="Done">'):(canResolve?'<input type="checkbox" class="cmt-resolve" data-k="'+this.esc(c.k)+'" title="Mark done">':'')}<span class="cmt-main"><span class="cmt-to cmt-to-${String(c.to||'none').toLowerCase()}">To ${this.esc(c.to||'Unassigned')}</span><span class="cmt-t">${this.esc(c.t)}</span><span class="cmt-m">— ${this.esc(c.by||'')} · ${String(c.ts||'').slice(0,10)}${admin?' <span class="cmt-del" data-k="'+this.esc(c.k)+'" title="Delete (admin)">✕</span>':''}</span></span></div>`;
  ...
  _levelOpenCmts(lv){const pre=lv+'||',o=this._actCmt||{};return Object.keys(o).filter(k=>k.indexOf(pre)===0&&o[k]&&!o[k].done).map(k=>{const p=k.split('||'),zmk=p[1],aid=p[2],v=o[k],z=((this.DATA.levels[lv]||{}).zones||[]).find(x=>this.zid(x)===zmk),am=this._actMeta().find(a=>a.id===aid),isRpt=!!v.report;return {k,...v,lv,zmk,aid,zone:isRpt?((v.reportCat||'')+' Report'):(z?z.label:(zmk.indexOf(lv+'|')===0?zmk.slice(lv.length+1):zmk)),activity:isRpt?('Report · '+(v.reportActivity||v.reportAid||'')):(am?am.label:aid)};}).sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.activity).localeCompare(String(b.activity))||String(a.ts||'').localeCompare(String(b.ts||'')));}
  _levelCmtPanel(lv){const rows=this._levelOpenCmts(lv);if(!rows.length)return `<div class="level-cmt-panel"><div class="level-cmt-panel-h">✓ No open comments on ${this.esc(lv)}</div></div>`;return `<div class="level-cmt-panel"><div class="level-cmt-panel-h"><span>Open comments · ${this.esc(lv)}</span><b>${rows.length}</b></div>${rows.map(c=>{const can=this.rwsIsAdmin()||this.rwsCanComment(c.lv,c.zmk)||this.rwsScopeOk(c.lv,c.zmk);return `<div class="level-cmt-row" data-k="${this.esc(c.k)}"><input type="checkbox" class="level-cmt-done" data-k="${this.esc(c.k)}" title="${can?'Mark done':'View only'}"${can?'':' disabled'}><button class="level-cmt-loc" data-k="${this.esc(c.k)}" title="Open this zone"><b>${this.esc(c.zone)}</b><span>${this.esc(c.activity)}</span></button><span class="cmt-to cmt-to-${String(c.to||'none').toLowerCase()}">To ${this.esc(c.to||'Unassigned')}</span><span class="level-cmt-text">${this.esc(c.t)}</span><span class="level-cmt-by">${this.esc(c.by||'')} · ${String(c.ts||'').slice(0,10)}</span></div>`;}).join('')}</div>`;}
  ...
  async rwsRenderAdmin(){
    ...
    const AL={EB:'Existing Basement',NB:'New Basement',MA:'Marine',RWS:'RWS · all Reports (view only)',REPEDIT:'Report Edit',CMT:'Comment (main + M28)',M28_OTHER:'M28 · L2/L3/L4/Ramp/TST',HIST:'History (snapshots)'};
    ...
    try{
      ...
      const AREAS=[['EB','Existing Basement'],['NB','New Basement'],['MA','Marine'],['RWS','RWS · all Reports (view only)'],['REPEDIT','Report Edit (combine with NB / EB / Marine)'],['CMT','Comment (main map + M28)'],['M28_OTHER','M28 · L2/L3/L4 · Ramp · TST · Hoarding'],['HIST','🕓 History / 历史快照 (view snapshots)']];
