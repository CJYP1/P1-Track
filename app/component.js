...
class Component extends DCLogic {
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      ...
      if(p<=0)return;const total=Number(this.actTotal(lv,zmk,a.id,a.total)||0),previousPlan=Math.max(0,cumPlan-p),previousDone=Math.max(0,cumDone-d),availableEarlierCredit=Math.max(0,previousDone-previousPlan),required=p+previousBacklog,currentApplied=Math.min(required,d),earlierCredit=Math.min(availableEarlierCredit,Math.max(0,required-currentApplied)),applied=currentApplied+earlierCredit,monthPct=required>0?Math.min(100,Math.round(applied/required*100)):null,behind=Math.max(0,required-applied),complete=behind<=0;
      rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,currentDone:d,currentApplied,done:applied,applied,earlierCredit,availableEarlierCredit,required,previousBacklog,monthPct,monthGap:behind,cumPlan,cumDone,total,unit:a.unit||'',complete,critical:!!z.crit,behind,status:complete?'complete':(applied>0?'actual':'behind')});
    });});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0)),cell='padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right';return `<div style="overflow-x:auto"><table style="width:100%;min-width:610px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Required Target</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Progress Applied</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Achieved</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${qty(r.required,r.unit,false)}<small style="display:block;color:#667085;font-size:7px">Plan ${this.esc(fmt(r.plan))}${r.previousBacklog>0?' + previous backlog '+this.esc(fmt(r.previousBacklog)):''}</small></td><td style="${cell};background:${r.applied>0?'#edf8f1':'transparent'}">${qty(r.applied,r.unit,r.applied>0)}<small style="display:block;color:#667085;font-size:7px">${this.esc(mon)} Actual ${this.esc(fmt(r.currentDone))}${r.earlierCredit>0?' + earlier completion '+this.esc(fmt(r.earlierCredit)):''}</small></td><td style="${cell}"><b style="font-size:11px;color:${r.complete?'#167846':'#c8102e'}">${r.monthPct}%</b></td><td style="${cell}"><b style="color:${r.behind>0?'#c8102e':'#167846'}">${r.behind>0?this.esc(fmt(r.behind)):'—'}</b>${r.behind>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:105px">${r.complete?'<b style="color:#167846">Plan + previous backlog achieved</b>':`<b style="color:#c8102e">${this.esc(mon)} outstanding</b>`}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
    ...
    [...ov.querySelectorAll('div,span')].forEach(el=>{const t=el.textContent.trim();if(!el.querySelector('div,span')&&t.startsWith('✓ Complete · overall scope done'))el.textContent=t.replace('✓ Complete · overall scope done + no monthly shortfall','✓ Plan + previous backlog achieved');else if(!el.querySelector('div,span')&&t==='No work meets both completion conditions yet.')el.textContent='No planned work has achieved its Required Target yet.';else if(!el.querySelector('div,span')&&t.startsWith('● In progress / monthly outstanding'))el.textContent=t.replace('● In progress / monthly outstanding','● Required Target outstanding');else if(!el.querySelector('div,span')&&t.startsWith('How this report decides “Complete”'))el.innerHTML=`<b>Included rows:</b> only Zones with a ${this.esc(mon)} Plan. <b>Required Target</b> = ${this.esc(mon)} Plan + previous backlog. <b>Progress Applied</b> = ${this.esc(mon)} Actual + any earlier completion ahead of the previous Plan. The target is achieved when Progress Applied covers the Required Target.`;else if(el.tagName==='SPAN'&&!el.querySelector('*')&&t.includes(' zones · '+mon+' Plan ')&&t.includes(' · Actual '))el.textContent=t.replace(' · Actual ',' · Applied ');});
