...
class Component extends DCLogic {
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      ...
      if(p<=0)return;
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||this.actAutoTotal(lv,zmk,a.id)||0),previousPlan=Math.max(0,cumPlan-p),previousDone=Math.max(0,cumDone-d),availableEarlierCredit=Math.max(0,previousDone-previousPlan),required=p,currentApplied=Math.min(required,d),earlierCredit=Math.min(availableEarlierCredit,Math.max(0,required-currentApplied)),applied=currentApplied+earlierCredit,behind=Math.max(0,required-applied),complete=behind<=0,monthPct=required>0?Math.min(100,Math.round(applied/required*100)):null;
      rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,currentDone:d,currentApplied,done:applied,applied,earlierCredit,availableEarlierCredit,required,previousBacklog,monthPct,monthGap:behind,cumPlan,cumDone,total,unit:a.unit||'',complete,completedEarlier:false,genuineComplete:false,critical:!!z.crit,behind,status:complete?'complete':(applied>0?'actual':'behind')});
    });});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    [...ov.querySelectorAll('div,span')].forEach(el=>{const t=el.textContent.trim();if(!el.querySelector('div,span')&&t.startsWith('✓ Complete · overall scope done'))el.textContent='✓ '+mon+' Plan achieved';else if(!el.querySelector('div,span')&&t==='No work meets both completion conditions yet.')el.textContent='No '+mon+' planned Zone has reached its monthly target yet.';else if(!el.querySelector('div,span')&&t.startsWith('● In progress / monthly outstanding'))el.textContent='● '+mon+' Plan outstanding';else if(!el.querySelector('div,span')&&t.startsWith('How this report decides “Complete”'))el.innerHTML=`<b>${this.esc(mon)} Monthly Summary detail:</b> only Zones with a ${this.esc(mon)} Plan are included. ${this.esc(mon)} Actual and work completed ahead of its earlier Plan can satisfy that monthly Target. Early-completed planned Zones appear under <b style="color:#167846">Complete</b>; historical completed Zones with no ${this.esc(mon)} Plan are not shown.`;else if(el.tagName==='SPAN'&&!el.querySelector('*')&&t.includes(' zones · '+mon+' Plan ')&&t.includes(' · Actual '))el.textContent=t.replace(' · Actual ',' · Actual / Early Done ');});
