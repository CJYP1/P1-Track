...
class Component extends DCLogic {
  ...
  componentDidMount(){
    const boot=(DATA,CAT,OVL,ZP,T)=>{ this.DATA=DATA; this.CAT=CAT; this.OVL=OVL; this.ZP=ZP;
      ...
      try{const src=this.DATA.levels.L4||this.DATA.levels.L3||this.DATA.levels.L2;if(src&&!this.DATA.levels.L5){const l5=JSON.parse(JSON.stringify(src));l5.title='Level 5';(l5.zones||[]).forEach(z=>{const old=String(z.mk||z.lid||z.label||'');z.mk=old.replace(/^(?:L4|L3|L2)\|/,'L5|');if(z.mk===old&&old.indexOf('|')<0)z.mk='L5|'+old;});this.DATA.levels.L5=l5;this.DATA.levelTotals.L5={...(this.DATA.levelTotals.L4||this.DATA.levelTotals.L3||this.DATA.levelTotals.L2||{})};if(this.DATA.order.indexOf('L5')<0){const at=this.DATA.order.indexOf('L4');this.DATA.order.splice(at>=0?at+1:this.DATA.order.length,0,'L5');}}}catch(_e3){console.error('L5 setup',_e3);}
  ...
  rwsCanResource(){return this.rwsIsAdmin()||this.rwsHasScope('RESOURCE');}
  ...
  _resourceEntry(lv,zmk){for(const t of this._resourceData().teams){const i=(t.zones||[]).find(x=>x.lv===lv&&x.zmk===zmk);if(i)return {team:t,item:i};}return null;}
  _resourceRenumber(t){(t.zones||[]).forEach((x,i)=>x.order=i+1);}
  ...
  _resourceToggleZone(lv,z){if(!this.rwsIsAdmin())return;const t=this._resourceTeam();if(!t)return;t.zones=t.zones||[];const zmk=z.mk||z.lid,at=t.zones.findIndex(x=>x.lv===lv&&x.zmk===zmk);if(at>=0)t.zones.splice(at,1);else{this._resourceData().teams.forEach(q=>{q.zones=(q.zones||[]).filter(x=>!(x.lv===lv&&x.zmk===zmk));this._resourceRenumber(q);});t.zones.push({lv,zmk,order:t.zones.length+1,formwork:null,concrete:null,total:null,workers:null});}this._resourceInspect={lv,zmk};this._resourceRenumber(t);this._resourceSave();this.render();this._renderResourcePanel();}
  _resourceNum(raw){const s=String(raw==null?'':raw).trim();return s===''?null:Math.max(0,Math.round((Number(s)||0)*100)/100);}
  openResourcePlanner(){if(!this.rwsCanResource()){this.rwsDeny('Resource permission is required.');return;}this._resourceMode=true;this._resourceTeam();this.buildMetrics();this.render();this._renderResourcePanel();}
  _closeResourcePlanner(){this._resourceMode=false;this._resourceInspect=null;const p=this.root.querySelector('#resourcePlannerPanel');if(p)p.remove();['#kpis','#progroll','#foot'].forEach(s=>{const e=this.root.querySelector(s);if(e)e.style.display='';});this.buildMetrics();this.render();}
  _renderResourcePanel(){if(!this._resourceMode)return;const side=this.root.querySelector('#side'),title=this.root.querySelector('#sideTitle'),kpis=this.root.querySelector('#kpis'),prog=this.root.querySelector('#progroll'),foot=this.root.querySelector('#foot'),p=this.root.querySelector('#sidebody');if(!side||!p)return;const old=this.root.querySelector('#resourcePlannerPanel');if(old)old.remove();p.id='sidebody';p.dataset.resource='1';if(title)title.textContent='🧰 Resource Planning';if(kpis)kpis.style.display='none';if(prog)prog.style.display='none';if(foot)foot.style.display='none';const d=this._resourceData(),admin=this.rwsIsAdmin(),t=this._resourceTeam(),colors=this._resourceColours(),n=v=>Number(v)||0,zs=t?(t.zones||[]):[],tot=zs.reduce((a,x)=>{a.f+=n(x.formwork);a.c+=n(x.concrete);a.t+=n(x.total);a.w+=n(x.workers);return a;},{f:0,c:0,t:0,w:0}),selected=this._resourceInspect&&this._resourceEntry(this._resourceInspect.lv,this._resourceInspect.zmk);
    ...
    const input=(x,k,label)=>admin?`<label style="display:block;color:var(--dim);font-size:8px;font-weight:800">${label}<input class="resource-qty" data-key="${k}" type="number" min="0" step="any" value="${x[k]==null?'':this.esc(x[k])}" placeholder="0" style="display:block;width:100%;box-sizing:border-box;margin-top:3px;padding:6px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt);font-weight:800"></label>`:`<div><small style="display:block;color:var(--dim);font-size:8px">${label}</small><b>${x[k]==null?'—':this.esc(x[k])}</b></div>`;
    const cards=zs.map((x,i)=>{const on=this._resourceInspect&&this._resourceInspect.lv===x.lv&&this._resourceInspect.zmk===x.zmk;return `<section class="resource-zone-card" data-i="${i}" style="margin-bottom:8px;padding:9px;border:${on?'2px':'1px'} solid ${on?t.color:'var(--line)'};border-radius:9px;background:${on?'color-mix(in srgb,'+t.color+' 8%,var(--panel))':'var(--panel)'}"><div style="display:flex;align-items:center;gap:7px"><b style="display:grid;place-items:center;width:25px;height:25px;border-radius:50%;background:${t.color};color:#fff">${i+1}</b><div style="min-width:0;flex:1"><b>${this.esc(x.lv)} · ${this.esc(this._resourceZoneLabel(x.lv,x.zmk))}</b><small style="display:block;color:${t.color};font-weight:800">${this.esc(t.name)} · Work order ${i+1}</small></div>${admin?`<button class="resource-up" title="Move earlier" ${i===0?'disabled':''}>↑</button><button class="resource-down" title="Move later" ${i===zs.length-1?'disabled':''}>↓</button><button class="resource-remove" title="Remove" style="color:var(--crit)">✕</button>`:''}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:9px">${input(x,'workers','Workers 人数')}${input(x,'formwork','Formwork 模板量')}${input(x,'concrete','Concrete 洋灰量')}${input(x,'total','Total 总量')}</div></section>`;}).join('')||`<div style="padding:24px 12px;text-align:center;color:var(--faint);border:1px dashed var(--line);border-radius:9px">${admin?'Select a Team, then click Zones on the map in work order.':'No Zones assigned to this team.'}</div>`;
    const selectedNote=this._resourceInspect?`<div style="margin:8px 0;padding:8px;border-radius:8px;background:var(--panel2);font-size:10px"><b>Selected:</b> ${this.esc(this._resourceInspect.lv)} · ${this.esc(this._resourceZoneLabel(this._resourceInspect.lv,this._resourceInspect.zmk))}${selected?`<span style="display:block;margin-top:3px;color:${selected.team.color};font-weight:900">${this.esc(selected.team.name)} · Order ${selected.item.order}</span>`:'<span style="display:block;margin-top:3px;color:var(--faint)">Click the Zone again after selecting a Team to assign it.</span>'}</div>`:'';
    p.innerHTML=`<div style="padding:10px 10px 18px"><div style="display:flex;align-items:center;gap:6px"><div style="font-size:10px;color:var(--dim);line-height:1.4;flex:1">${admin?'Choose a Team, then click Zones on the map in the required work order. Edit quantities here.':'Click a coloured Zone to view its Team and resources.'}</div><button class="hbtn" id="resourceClose" style="padding:5px 8px">Exit</button></div><div style="display:flex;gap:5px;flex-wrap:wrap;margin:10px 0">${tabs}${admin?'<button class="hbtn" id="resourceAddTeam" style="padding:5px 8px">+ Team</button>':''}</div>${selectedNote}${t?`<div style="display:flex;align-items:center;gap:7px;margin:9px 0;padding:8px;background:var(--panel2);border-radius:8px">${admin?`<input id="resourceTeamColor" type="color" value="${t.color}" title="Team colour" style="width:30px;height:30px;padding:1px;border:1px solid var(--line);border-radius:6px"><input id="resourceTeamName" value="${this.esc(t.name)}" style="min-width:0;flex:1;font-weight:900;padding:6px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt)">`:`<span style="width:12px;height:12px;border-radius:50%;background:${t.color}"></span><b style="flex:1">${this.esc(t.name)}</b>`}<span style="font-size:9px;color:var(--dim)">${zs.length} Zones</span>${admin&&d.teams.length>1?'<button id="resourceDeleteTeam" style="border:0;background:transparent;color:var(--crit);cursor:pointer">Delete</button>':''}</div>${cards}<div style="padding:9px;border-top:3px solid ${t.color};background:var(--panel2);border-radius:8px;font-size:10px"><b>TEAM TOTAL</b><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:5px"><span>Workers <b>${this.fmt(tot.w)}</b></span><span>Total <b>${this.fmt(tot.t)}</b></span><span>Formwork <b>${this.fmt(tot.f)}</b></span><span>Concrete <b>${this.fmt(tot.c)}</b></span></div></div>`:'<div class="empty">No teams yet.</div>'}</div>`;
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      ...
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||this.actAutoTotal(lv,zmk,a.id)||0),previousPlan=Math.max(0,cumPlan-p),previousDone=Math.max(0,cumDone-d),previousBacklog=Math.max(0,previousPlan-previousDone);
      ...
      const backlogRecovery=p<=0&&previousBacklog>0&&d>0;
      if(p<=0&&!backlogRecovery)return;
      const required=backlogRecovery?previousBacklog:p,availableEarlierCredit=backlogRecovery?0:Math.max(0,previousDone-previousPlan),currentApplied=Math.min(required,d),earlierCredit=backlogRecovery?0:Math.min(availableEarlierCredit,Math.max(0,required-currentApplied)),applied=currentApplied+earlierCredit,behind=Math.max(0,required-applied),complete=behind<=0,monthPct=required>0?Math.min(100,Math.round(applied/required*100)):null,scopeTotal=total>0?total:Math.max(cumPlan,cumDone),sitePct=scopeTotal>0?Math.min(100,Math.round(cumDone/scopeTotal*100)):0,overallOutstanding=Math.max(0,scopeTotal-cumDone),overallComplete=scopeTotal>0&&overallOutstanding<=0,weekActual=this._reportWeekActual(lv,zmk,a.id,z,mon,refs);
      const reportZone=backlogRecovery?zone+' · BACKLOG RECOVERY ('+previousBacklog+' in)':zone;
      rows.push({cat,lv,zmk,zone:reportZone,zoneLabel:zone,aid:a.id,name:a.label,plan:p,currentDone:d,currentApplied,done:applied,applied,earlierCredit,availableEarlierCredit,required,previousBacklog,backlogRecovery,monthPct,monthGap:behind,cumPlan,cumDone,total,scopeTotal,sitePct,weekActual,overallOutstanding,overallComplete,unit:a.unit||'',complete,completedEarlier:false,genuineComplete:false,critical:!!z.crit,behind,status:complete?'complete':(applied>0?'actual':'behind')});
    });});
  ...
  _exportCombinedMonthlyPng(cat,lv,mon){
    ...
    rows.forEach(r=>(by[r.aid]||(by[r.aid]={name:r.name,rows:[]})).rows.push(r));const acts=Object.keys(by).sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);return (ai<0?999:ai)-(bi<0?999:bi)||by[a].name.localeCompare(by[b].name);}),W=1500,rowH=42,headH=38,H=Math.min(30000,165+acts.length*(headH+8)+rows.length*rowH),cv=document.createElement('canvas');cv.width=W;cv.height=H;const x=cv.getContext('2d'),fmt=n=>Math.round(Number(n)||0).toLocaleString(),week=this._reportWeekBounds(mon),weekLabel=week?week.start.slice(8)+'–'+week.end.slice(8)+' '+mon.split("'")[0]:'This Week';
    x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle=meta.color;x.fillRect(0,0,W,80);x.fillStyle='#fff';x.font='700 27px Arial';x.fillText('P1 Waterfront · '+meta.name+' · '+lv,34,35);x.font='700 16px Arial';x.fillText(mon+' · Simple progress summary',34,62);const cols=[34,520,745,980,1230];let y=108;x.font='700 13px Arial';x.fillStyle='#475467';['Level · Zone','Plan','Cumulative Completed','This Week Actual ('+weekLabel+')','Outstanding'].forEach((h,i)=>x.fillText(h,cols[i],y));y+=20;x.strokeStyle='#cfd7e2';x.beginPath();x.moveTo(28,y);x.lineTo(W-28,y);x.stroke();
    acts.forEach(aid=>{const g=by[aid];if(y+headH>H)return;x.fillStyle='#e8edf3';x.fillRect(28,y,W-56,headH);x.fillStyle='#202938';x.font='700 16px Arial';x.fillText(g.name,40,y+25);y+=headH;g.rows.sort((a,b)=>(b.overallComplete-a.overallComplete)||String(a.zone).localeCompare(String(b.zone))).forEach(r=>{if(y+rowH>H)return;x.fillStyle=r.overallComplete?'#edf8f1':'#fff';x.fillRect(28,y,W-56,rowH);x.fillStyle='#202938';x.font='700 13px Arial';x.fillText(r.lv+' · '+(r.zoneLabel||r.zone)+(r.critical?'  CRITICAL':''),cols[0],y+25);x.font='13px Arial';x.fillText(fmt(r.scopeTotal)+(r.unit?' '+r.unit:''),cols[1],y+25);x.fillStyle=r.cumDone>0?'#167846':'#202938';x.fillText(fmt(r.cumDone)+(r.unit?' '+r.unit:''),cols[2],y+25);x.fillStyle=r.weekActual>0?'#167846':'#667085';x.fillText(r.weekActual==null?'—':fmt(r.weekActual)+(r.unit?' '+r.unit:''),cols[3],y+25);x.fillStyle=r.overallOutstanding>0?'#c8102e':'#167846';x.fillText(r.overallOutstanding>0?fmt(r.overallOutstanding)+(r.unit?' '+r.unit:''):'—',cols[4],y+25);x.strokeStyle='#e4e8ee';x.beginPath();x.moveTo(28,y+rowH);x.lineTo(W-28,y+rowH);x.stroke();y+=rowH;});y+=8;});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{
      ...
      const body=order.map(lv=>`<tr><td colspan="5" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>{
        return `<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open ${this.esc(r.lv)} · ${this.esc(r.zoneLabel||r.zone)} details">${this.esc(r.lv)} · ${this.esc(r.zoneLabel||r.zone)} ↗</button>${r.backlogRecovery?'<small style="display:block;color:#b54708;font-size:7px;font-weight:900;margin-top:2px">BACKLOG WORK</small>':''}${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${qty(r.scopeTotal,r.unit,false)}</td><td style="${cell};background:${r.cumDone>0?'#edf8f1':'transparent'}">${qty(r.cumDone,r.unit,r.cumDone>0)}</td><td style="${cell};background:#f4f7fb">${r.weekActual==null?'<b style="font-size:11px;color:#98a2b3">—</b>':qty(r.weekActual,r.unit,r.weekActual>0)}</td><td style="${cell}"><b style="font-size:11px;color:${r.overallOutstanding>0?'#c8102e':'#167846'}">${r.overallOutstanding>0?this.esc(fmt(r.overallOutstanding)):'—'}</b>${r.overallOutstanding>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td></tr>`;
      }).join('')).join('');
      return `<div style="overflow-x:auto"><table style="width:100%;min-width:650px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Level · Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Plan</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Cumulative Completed</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">This Week Actual<small style="display:block;font-size:7px;font-weight:700">${this.esc(weekLabel)}</small></th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th></tr></thead><tbody>${body}</tbody></table></div>`;
    };
  ...
  openLookAhead(){
    ...
    if(this._reportCombined){ov.innerHTML='<div style="max-width:520px;margin:90px auto;text-align:center;color:var(--dim);font-size:14px"><b style="display:block;color:var(--txt);font-size:17px;margin-bottom:8px">Loading Combined Status…</b>Preparing the latest plan and progress data.</div>';ov.style.display='block';setTimeout(()=>{if(this._reportCombined&&ov.style.display!=='none')this._safeRenderCombinedReport(ov,cats,close);},0);return;}
  ...
  render(){
    ...
    L.zones.forEach((z,i)=>{
      ...
      let op=vis?(_focusOnly?0.58:(this.colorMode==='area'?0.5:0.72)):0.05;
      if(vis&&this._resourceMode)op=this._resourceEntry(this.curLevel,z.mk||z.lid)?0.68:0.22;
      let planst='';
      ...
      s+=`<polygon class="zone${vis?'':' dim'}${crit}${planst}${_aggPicked?' aggpick':''}" data-i="${i}" points="${pts}" fill="${this.zoneFill(z)}" fill-opacity="${op}"${_resInspect?' style="stroke:#111827;stroke-width:1500px;filter:drop-shadow(0 0 900px #fff)"':''}/>`;
    });
    ...
    this.svg.querySelectorAll('.subz').forEach(el=>{
      ...
      el.addEventListener('click',ev=>{if(this._placingCol)return;ev.stopPropagation();const [cls,i]=el.dataset.sk.split('|');const kind=cls.replace('sub','');
        ...
        if(this._resourceMode&&pick){const zmk=pick.mk||pick.lid,re=this._resourceEntry(this.curLevel,zmk);this._resourceInspect={lv:this.curLevel,zmk};if(this.rwsIsAdmin()&&!re){this._resourceToggleZone(this.curLevel,pick);return;}if(re)this._resourceTeamId=re.team.id;this.render();this._renderResourcePanel();return;}
  ...
  drawLegend(){
    ...
    if(this._resourceMode){
      const ts=this._resourceData().teams;s+='<div style="font-weight:800;color:var(--txt);margin-bottom:3px">Resource · Team Legend</div><div style="font-size:9px;color:var(--faint);margin-bottom:5px">Zone colour = assigned team · number = work order<br>W = Workers · F = Formwork · C = Concrete · T = Total</div>';
      ts.forEach(t=>{const n=(t.zones||[]).filter(x=>x.lv===this.curLevel).length;s+=`<div class="lr"><span class="sw" style="background:${t.color}"></span><b>${this.esc(t.name)}</b><span style="margin-left:auto;color:var(--faint)">${n} zone${n===1?'':'s'}</span></div>`;});
      s+='<div class="lr"><span class="sw" style="background:#d9dee7;opacity:.45"></span>Unassigned</div>';
  ...
  elRows(lv,z,type,items,metaFn,empty,critFn){if(!items||!items.length)return `<div class="empty">${empty}</div>`;
    return items.map(x=>{const id=(typeof x==='string')?x:x.id;const key=this.ekey(lv,z,type,id);const crit=critFn&&critFn(x);
      ...
      const idHtml=(type==='col')?`<span class="id collink" data-colid="${this.esc(id)}" title="Show this column on the plan">${this.esc(displayId)}</span>`:`<span class="id">${this.esc(displayId)}</span>`;
  ...
  buildMetrics(){
    ...
    const modes=[['area','Area',(this.CAT.NB).c],['plan','Planned (month)',this.cssvar('--accent')]];if(this.rwsCanResource())modes.push(['resource','Resource','#3157d5']);
    const seg=document.createElement('div');seg.className='modeseg';
    modes.forEach(([k,lab,dot])=>{const btn=document.createElement('button'),on=k==='resource'?this._resourceMode:(!this._resourceMode&&k===this.colorMode);btn.className=on?'on':'';btn.innerHTML=`<span class="dt" style="background:${dot}"></span>${lab}`;btn.addEventListener('click',()=>{if(k==='resource'){this.openResourcePlanner();return;}if(this._resourceMode){this._resourceMode=false;this._resourceInspect=null;['#kpis','#progroll','#foot'].forEach(s=>{const e=this.root.querySelector(s);if(e)e.style.display='';});}this.colorMode=k;this.buildMetrics();this.render();});seg.appendChild(btn);});
    mc.appendChild(seg);
  ...
  bindGlobal(){
    ...
    {const _la=this.root.querySelector('#openLookAhead');if(_la)_la.addEventListener('click',()=>{this._reportCombined=true;this._reportCombinedArea='NB';this._reportEditing=false;this.openLookAhead();});}
