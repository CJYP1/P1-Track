...
class Component extends DCLogic {
  ...
  _cmtBtn(lv,zmk,aid){const n=this.visibleActCmts(lv,zmk,aid).filter(c=>!c.done).length;return `<span class="actcmt-btn" data-a="${aid}" title="Comments" style="cursor:pointer;position:relative;display:inline-flex;align-items:center;justify-content:center;margin-left:8px">${this._cmtBubbleSVG()}${n?`<span style="position:absolute;top:-8px;right:-8px;background:#e5343a;color:#fff;font-size:11px;font-weight:800;min-width:17px;height:17px;line-height:17px;text-align:center;border-radius:9px;box-shadow:0 0 0 2px var(--panel,#fff);padding:0 3px">${n}</span>`:''}</span>`;}
  _cmtPanel(lv,zmk,aid,reportCat){if(this._cmtOpen!==lv+'||'+zmk+'||'+aid)return '';const all=this.visibleActCmts(lv,zmk,aid);const admin=this.rwsIsAdmin();const canAdd=admin||this.rwsCanComment(lv,zmk);   // 所有位置（包括 Report）统一使用原来的 CMT 评论权限
    ...
    const row=(c,res)=>{const canResolve=this._canResolveActCmt(c,lv,zmk);return `<div class="cmtrow${res?' cmt-done':''}">${res?(admin?'<input type="checkbox" class="cmt-unresolve" data-k="'+this.esc(c.k)+'" checked title="Reopen (admin)">':'<input type="checkbox" checked disabled title="Done">'):(canResolve?'<input type="checkbox" class="cmt-resolve" data-k="'+this.esc(c.k)+'" title="Mark done">':'')}<span class="cmt-main"><span class="cmt-to cmt-to-${String(c.to||'none').toLowerCase()}">To ${this.esc(c.to||'Unassigned')}</span><span class="cmt-t">${this.esc(c.t)}</span><span class="cmt-m">— ${this.esc(c.by||'')} · ${String(c.ts||'').slice(0,10)}${c.edited?' · edited':''}${admin?' <span class="cmt-edit" data-k="'+this.esc(c.k)+'" title="Edit recipient and comment (admin)">✎</span> <span class="cmt-del" data-k="'+this.esc(c.k)+'" title="Delete (admin)">✕</span>':''}</span></span></div>`;};
  ...
  _levelOpenCmts(lv){const pre=lv+'||',o=this._actCmt||{};return Object.keys(o).filter(k=>k.indexOf(pre)===0&&o[k]&&!o[k].done).map(k=>{const p=k.split('||'),zmk=p[1],aid=p[2],v=o[k];if(!this._canSeeActCmt(v,lv,zmk))return null;const z=((this.DATA.levels[lv]||{}).zones||[]).find(x=>this.zid(x)===zmk),am=this._actMeta().find(a=>a.id===aid),isRpt=!!v.report;return {k,...v,lv,zmk,aid,zone:isRpt?((v.reportCat||'')+' Report'):(z?z.label:(zmk.indexOf(lv+'|')===0?zmk.slice(lv.length+1):zmk)),activity:isRpt?('Report · '+(v.reportActivity||v.reportAid||'')):(am?am.label:aid)};}).filter(Boolean).sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.activity).localeCompare(String(b.activity))||String(a.ts||'').localeCompare(String(b.ts||'')));}
  _levelCmtPanel(lv){const rows=this._levelOpenCmts(lv);if(!rows.length)return `<div class="level-cmt-panel"><div class="level-cmt-panel-h">✓ No open comments on ${this.esc(lv)}</div></div>`;return `<div class="level-cmt-panel"><div class="level-cmt-panel-h"><span>Open comments · ${this.esc(lv)}</span><b>${rows.length}</b></div>${rows.map(c=>{const can=this._canResolveActCmt(c,c.lv,c.zmk);return `<div class="level-cmt-row" data-k="${this.esc(c.k)}"><input type="checkbox" class="level-cmt-done" data-k="${this.esc(c.k)}" title="${can?'Mark done':'View only'}"${can?'':' disabled'}><button class="level-cmt-loc" data-k="${this.esc(c.k)}" title="Open this zone"><b>${this.esc(c.zone)}</b><span>${this.esc(c.activity)}</span></button><span class="cmt-to cmt-to-${String(c.to||'none').toLowerCase()}">To ${this.esc(c.to||'Unassigned')}</span><span class="level-cmt-text">${this.esc(c.t)}</span><span class="level-cmt-by">${this.esc(c.by||'')} · ${String(c.ts||'').slice(0,10)}${this.rwsIsAdmin()?' · <span class="cmt-edit" data-k="'+this.esc(c.k)+'">✎ Edit</span>':''}</span></div>`;}).join('')}</div>`;}
  _openLevelCmt(k){const p=String(k||'').split('||'),v=(this._actCmt||{})[k];if(v&&v.report){this._reportCat=v.reportCat||this._reportCat;this._cmtOpen=p.slice(0,3).join('||');this.openLookAhead();return;}const lv=p[0],zmk=p[1];if(this._zoneFindZone(lv,zmk)){this.gotoZone(lv,zmk);return;}if(lv==='L1'&&zmk.indexOf('L1|')===0){const lab=zmk.slice(3);for(const kind of ['P','C']){const i=((this.SUBZONES.L1||{})[kind]||[]).findIndex(x=>String(x.label)===lab);if(i>=0){if(lv!==this.curLevel)this.setLevel(lv);this.selectSubzone(kind,i);return;}}}}
  ...
  _reportCmtBtn(ctx){if(!ctx.zmk)return '';const n=this.visibleActCmts(ctx.lv,ctx.zmk,ctx.aid).filter(c=>!c.done).length;return `<button type="button" class="rpt-cmt-open" data-lv="${this.esc(ctx.lv)}" data-zmk="${this.esc(ctx.zmk)}" data-a="${this.esc(ctx.aid)}" title="Report comments" style="position:relative;margin-top:7px;border:1px solid rgba(255,255,255,.65);border-radius:7px;background:rgba(255,255,255,.12);padding:3px 7px;cursor:pointer">${this._cmtBubbleSVG()}${n?`<b style="position:absolute;right:-8px;top:-8px;background:#e5343a;color:#fff;border-radius:10px;min-width:18px;height:18px;line-height:18px;font-size:10px">${n}</b>`:''}</button>`;}
  ...
  buildList(){
    ...
    const _openCmts=this._levelOpenCmts(this.curLevel),_pmN=_openCmts.filter(c=>c.to==='PM').length,_plN=_openCmts.filter(c=>c.to==='Planning').length;
    this.root.querySelector('#sideTitle').innerHTML=`<span>${this.esc(_viewTitle)}</span><span class="side-hd-right"><span class="pill">${_viewCount} zones</span><button class="level-cmt-badge${_openCmts.length?' has-open':''}" title="Open comments · PM ${_pmN} · Planning ${_plN}">${_openCmts.length}</button></span>`;
    ...
    const _cBadge=this.root.querySelector('.level-cmt-badge');if(_cBadge)_cBadge.addEventListener('click',()=>{this._levelCmtOpen=!this._levelCmtOpen;this.buildList();});
  ...
  buildRail(){
    ...
    VIS.forEach(lv=>{const L=this.DATA.levels[lv];
      ...
      const cn=this._levelOpenCmts(lv).length;
      b.innerHTML=`${cn?`<span class="rail-cmt-badge" title="${cn} open comments">${cn}</span>`:''}<div class="code">${lv}</div><div class="nm">${this.esc(L.title.replace(/\(.*/,'').trim())}</div><div class="rpb"><i style="width:${lp.avg}%"></i></div><div class="rpct">${lp.avg}%</div>`;
