...
class Component extends DCLogic {
  ...
  _delayQtyStats(lv,z){if(!z)return null;const zmk=z.mk||z.lid||'',M=this.ACT_MONTHS||[],now=new Date(),lab=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][now.getMonth()]+"'"+String(now.getFullYear()).slice(2),mi=M.indexOf(lab);if(mi<0)return null;const dim=new Date(now.getFullYear(),now.getMonth()+1,0).getDate(),monthFrac=Math.max(0,Math.min(1,now.getDate()/dim)),parseMonth=s=>{const m=String(s||'').match(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)'(\d{2})$/);if(!m)return null;const mons={Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};return new Date(Date.UTC(2000+(+m[2]),mons[m[1]],1));},asOf=Date.UTC(now.getFullYear(),now.getMonth(),now.getDate()),DAY=86400000,items=[];
    (this._actList(lv,z)||[]).filter(a=>(a.custom||this._actApplies(a.id,lv,z))&&!this.actHidden(lv,zmk,a.id)).forEach(a=>{let ptd=0,done=0,first=null;for(let i=0;i<=mi;i++){const p=Number(this.actPlan(lv,zmk,a.id,M[i])||0),d=Number(this.actDoneMonth(lv,zmk,a.id,M[i])||0);if(p>0&&!first)first=parseMonth(M[i]);ptd+=p*(i===mi?monthFrac:1);done+=d;}if(ptd<=0||!first)return;const ratio=done/ptd,shortfall=1-ratio,span=Math.max(1,Math.round((asOf-first.getTime())/DAY)+1),days=span*shortfall;items.push({aid:a.id,label:a.label,plan:ptd,done,ratio,days});});
    if(!items.length)return null;const avgDays=items.reduce((n,x)=>n+x.days,0)/items.length,avgRatio=items.reduce((n,x)=>n+x.ratio,0)/items.length;return {days:Math.round(avgDays),progressPct:Math.max(0,Math.round(avgRatio*100)),behindPct:Math.max(0,Math.round((1-avgRatio)*100)),activities:items.length,items};}
  _delayBaseDays(lv,z){const q=this._delayQtyStats(lv,z);return q?q.days:null;}
  ...
  openDelayAdmin(){if(!this.rwsIsAdmin()){this.rwsDeny('Only admin can edit Delay days.');return;}const old=document.getElementById('__delayAdmin');if(old)old.remove();
    ...
    const body=rows.map((r,i)=>{let raw=Object.prototype.hasOwnProperty.call(map,r.key)?map[r.key]:'';if(raw&&typeof raw==='object')raw=raw.days!=null?raw.days:raw.delay_days;if(raw==null)raw='';if(raw!==''&&Number.isFinite(Number(raw)))raw=Math.round(Number(raw));const qs=this._delayQtyStats(r.lv,r.z),base=qs?qs.days:null,eff=raw!==''?Number(raw):base,dv=eff==null?null:this._delayView(eff),auto=base==null?'—':`<b style="color:${this._delayView(base).c}">${this.esc(this._delayView(base).txt)}</b><small style="display:block;margin-top:3px">Actual ${this.esc(qs.progressPct)}% of plan-to-date · ${this.esc(qs.activities)} activit${qs.activities===1?'y':'ies'}</small>`;return `<tr data-q="${this.esc((r.lv+' '+r.label+' '+r.cat+' '+r.group).toLowerCase())}"><td>${this.esc(r.lv)}</td><td><b>${this.esc(r.label)}</b>${r.group?`<small>${this.esc(r.group)}</small>`:''}</td><td>${this.esc(cats[r.cat]||r.cat)}</td><td class="delay-base" data-days="${base==null?'':this.esc(String(base))}">${auto}</td><td><div class="delay-edit"><input class="delay-admin-in" type="number" step="1" data-key="${this.esc(r.key)}" value="${this.esc(String(raw))}" placeholder="Auto"><span>days</span><button type="button" class="delay-clear" title="Clear override and use quantity-based Auto">↺</button></div></td><td class="delay-result" style="color:${dv?dv.c:'#667085'}">${dv?this.esc(dv.txt):'—'}</td></tr>`;}).join('');
    const ov=document.createElement('div');ov.id='__delayAdmin';ov.innerHTML=`<div class="delay-admin-box"><div class="delay-admin-head"><div><b>Delay Table</b><span>Quantity-based Auto · positive = delayed · negative = ahead · blank Override = Auto</span></div><button class="hbtn" id="__delayClose">Close ✕</button></div><div class="delay-admin-tools"><input id="__delayFind" placeholder="Find level or zone…"><span>${rows.length} zones</span></div><div class="delay-admin-scroll"><table><thead><tr><th>Level</th><th>Zone</th><th>Area</th><th>Quantity-based Auto</th><th>Override</th><th>Map display</th></tr></thead><tbody>${body}</tbody></table></div><div class="delay-admin-foot"><span>Auto uses each Activity's plan-to-date versus cumulative actual quantity. Different units are normalised separately. An Override replaces Auto; clear it to restore live calculation.</span><button class="hbtn primary" id="__delaySave">Save Delay Table</button></div></div>`;
