...
class Component extends DCLogic {
  ...
  openBimLinks(){
    ...
    ov.innerHTML=`<div style="background:var(--panel);color:var(--txt);border:1px solid var(--line);border-radius:14px;width:min(920px,96vw);max-height:90vh;display:flex;flex-direction:column;box-shadow:0 24px 60px rgba(0,0,0,.4)"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px 18px;border-bottom:1px solid var(--line)"><div><div style="font-size:15px;font-weight:900">🔗 BIM Model Links</div><div style="font-size:9.5px;color:var(--faint);margin-top:2px">Saved online · ${manage?'Admin and Planning can add or edit':'select a model to open'}</div></div><button class="hbtn" id="__bimClose">Close ✕</button></div><div style="display:grid;grid-template-columns:minmax(180px,1fr) minmax(135px,190px) auto;gap:8px;align-items:center;padding:10px 18px;border-bottom:1px solid var(--line);background:var(--panel2)"><input id="__bimSearch" type="search" placeholder="Search name, category or description…" aria-label="Search BIM links" style="min-width:0;padding:7px 9px;border:1px solid var(--line);border-radius:8px;background:var(--panel);color:var(--txt);font:inherit"><select id="__bimFilter" aria-label="Filter BIM links by category" style="min-width:0;padding:7px 9px;border:1px solid var(--line);border-radius:8px;background:var(--panel);color:var(--txt);font:inherit"><option value="">All categories</option></select><span id="__bimCount" style="white-space:nowrap;font-size:10px;font-weight:800;color:var(--dim)"></span><datalist id="__bimCats"></datalist></div><div id="__bimBody" style="overflow:auto;padding:14px 18px"></div>${manage?`<div style="display:flex;justify-content:space-between;gap:8px;padding:12px 18px;border-top:1px solid var(--line)"><button class="hbtn" id="__bimAdd">+ Add link</button><button class="hbtn primary" id="__bimSave" style="padding:7px 18px">Save links</button></div>`:''}</div>`;
