...
class Component extends DCLogic {
  ...
  openBimLinks(){
    ...
    const render=()=>{
      ...
      else{const grouped={};shown.forEach(({x})=>{const c=cleanCat(x.category);(grouped[c]||(grouped[c]=[])).push({...x,url:this._bimLinkUrl(x.url)});});const names=Object.keys(grouped).sort((a,b)=>a.localeCompare(b));body.innerHTML=names.length?names.map(c=>`<section style="margin-bottom:17px"><div style="display:flex;align-items:center;gap:8px;margin:0 0 7px;font-size:11px;font-weight:900;color:var(--dim)"><span>${this.esc(c)}</span><span style="font-size:9px;color:var(--faint)">${grouped[c].length}</span></div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(235px,1fr));gap:8px">${grouped[c].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))).map(x=>`<a href="${this._bimAttr(x.url)}" target="_blank" rel="noopener noreferrer" style="display:flex;flex-direction:column;justify-content:space-between;min-height:74px;text-decoration:none;color:inherit;border:1px solid var(--line);border-radius:10px;padding:10px 12px;background:var(--panel2)"><div><b style="font-size:12px;color:var(--accent)">${this.esc(x.name||'BIM model')}</b>${x.note?`<div style="font-size:10px;color:var(--dim);margin-top:4px;line-height:1.35">${this.esc(x.note)}</div>`:''}</div><span style="align-self:flex-end;margin-top:7px;font-size:10px;font-weight:800;color:var(--accent)">Open ↗</span></a>`).join('')}</div></section>`).join(''):'<div class="empty">'+(links.length?'No links match this search.':'No BIM links have been added yet.')+'</div>';}
    };
