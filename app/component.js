...
class Component extends DCLogic {
  ...
  _reportWeekActual(lv,zmk,aid,z,mon){
    const bounds=this._reportWeekBounds(mon);if(!bounds)return null;const refs=this._activityElemRefs(lv,zmk,aid,z);if(!refs.length)return null;let done=0;
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0),carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null,previousBacklog=carry&&carry.carryIn>0?carry.carryIn:0;
      let cumPlan=0,cumDone=0;for(let j=0;j<=mi;j++){cumPlan+=Number(this.actPlan(lv,zmk,a.id,this.ACT_MONTHS[j])||0);cumDone+=Number(this.actDoneMonth(lv,zmk,a.id,this.ACT_MONTHS[j])||0);}
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||this.actAutoTotal(lv,zmk,a.id)||0),previousPlan=Math.max(0,cumPlan-p),previousDone=Math.max(0,cumDone-d);
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    [...ov.querySelectorAll('div,span')].forEach(el=>{const t=el.textContent.trim();if(!el.querySelector('div,span')&&t.startsWith('✓ Complete · overall scope done'))el.textContent='✓ '+mon+' Plan achieved / backlog cleared';else if(!el.querySelector('div,span')&&t==='No work meets both completion conditions yet.')el.textContent='No '+mon+' planned Zone or recovered backlog is complete yet.';else if(!el.querySelector('div,span')&&t.startsWith('● In progress / monthly outstanding'))el.textContent='● '+mon+' Plan / previous backlog outstanding';else if(!el.querySelector('div,span')&&t.startsWith('How this report decides “Complete”'))el.innerHTML=`<b>${this.esc(mon)} Monthly Summary detail:</b> Zones with a ${this.esc(mon)} Plan are shown, plus overdue earlier work actually completed during ${this.esc(mon)}. Those recovery rows are marked <b style="color:#b54708">BACKLOG RECOVERY</b>: Target Only remains 0, while Actual, recovery %, and remaining backlog show the catch-up made this month.`;else if(el.tagName==='SPAN'&&!el.querySelector('*')&&t.includes(' zones · '+mon+' Plan ')&&t.includes(' · Actual '))el.textContent=t.replace(' · Actual ',' · Actual / Backlog Recovered ');});
