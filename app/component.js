...
class Component extends DCLogic {
  ...
  _rpRollup(){const PROG=[["Apr","'26"],["May","'26"],["Jun","'26"],["Jul","'26"],["Aug","'26"],["Sep","'26"],["Oct","'26"],["Nov","'26"],["Dec","'26"],["Jan","'27"],["Feb","'27"],["Mar","'27"],["Apr","'27"],["May","'27"]];
    ...
    const agg=(ok)=>{let planned=0,done=0;const dm=new Array(14).fill(0);let before=0;const byAct={};const dmp=new Array(14).fill(0);let beforeP=0;
      ...
      return {planned:Math.round(planned),done:Math.round(done),doneMonth:dm.map(x=>Math.round(x)),cum,cumP,byAct};};
    ...
    return {R:{totalPlanned:all.planned,totalDone:all.done,doneMonth:all.doneMonth,cum:all.cum,overallPct:all.planned?Math.round(all.done/all.planned*100):0,critPlanned:crit.planned,critDone:crit.done,critDoneMonth:crit.doneMonth,critCum:crit.cum,byCat:mkCat(all.byAct),critByCat:mkCat(crit.byAct),critPct:crit.planned?Math.round(crit.done/crit.planned*100):0,hasCrit:crit.planned>0,hasData:all.done>0,byArea:_byArea,delayDays:_dAll.delayDays,behindQty:_dAll.behindQty,planToDate:_dAll.planToDate,doneToDate:_dAll.doneToDate,critDelayDays:_dCrit.delayDays,critBehindQty:_dCrit.behindQty},asOf};}
  ...
  _showRPL(){this._rpView=true;this._rpActive='__RPL';const c=this.root.querySelector('#center');if(c)c.style.position='relative';this.root.querySelectorAll('.rbtn').forEach(x=>x.classList.toggle('on',x.dataset.lv==='__RPL'));const hf=this.root.querySelector('#rpframe');if(hf)hf.style.display='none';let f=this.root.querySelector('#rplframe');if(!f){f=document.createElement('iframe');f.id='rplframe';f.setAttribute('sandbox','allow-scripts allow-popups');f.style.cssText='position:absolute;inset:0;width:100%;height:100%;border:0;background:#eef1f5;z-index:60';c.appendChild(f);}const rp=this._rpRollup();const b=this.root.querySelector('#rpLinkedTpl');let tpl=b?this._b64ToUtf8(b.textContent.trim()):'';const inj='<scr'+'ipt>window.__ROLLUP='+JSON.stringify(rp.R)+';window.__ASOF='+JSON.stringify(rp.asOf)+';</scr'+'ipt>';
    const banner='<scr'+'ipt>(function(){var R=window.__ROLLUP||{},A=window.__ASOF||"";function mk(){var cd=R.critDelayDays||0,od=R.delayDays||0;var col=cd>0?"#c0392b":"#0f7a52";var areas=(R.byArea||[]).map(function(a){return a.label+" "+(a.delayDays>0?("<b style=\\"color:#c0392b\\">"+a.delayDays+"d</b>"):"<b style=\\"color:#0f7a52\\">on track</b>");}).join(" &nbsp;·&nbsp; ");var el=document.createElement("div");el.style.cssText="background:#fff;border-bottom:2px solid "+col+";padding:9px 18px;font:600 12.5px system-ui,-apple-system,Segoe UI,sans-serif;color:#222;line-height:1.55";el.innerHTML="<b style=\\"color:"+col+"\\">Schedule delay: "+(cd>0?("~"+cd+" days behind"):"on track")+"</b> &nbsp;(critical path · as of "+A+") &nbsp;·&nbsp; overall ~"+od+"d &nbsp;·&nbsp; planned-to-date "+(R.planToDate||0)+" · done "+(R.doneToDate||0)+" · behind "+(R.critBehindQty||0)+"<div style=\\"margin-top:3px;font-weight:500;color:#555\\">By area: "+areas+"</div>";document.body.insertBefore(el,document.body.firstChild);}if(document.body)mk();else document.addEventListener("DOMContentLoaded",mk);})();</scr'+'ipt>';
    tpl=tpl.replace('<body>','<body>'+inj+banner);f.srcdoc=tpl;f.style.display='block';
