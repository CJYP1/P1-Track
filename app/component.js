...
class Component extends DCLogic {
  ...
  _resourceZoneLabel(lv,zmk){const L=this.DATA.levels[lv],z=L&&(L.zones||[]).find(x=>(x.mk||x.lid)===zmk);return z?z.label:zmk;}
  ...
  openResourcePlanner(){if(!this.rwsCanResource()){this.rwsDeny('Resource permission is required.');return;}this._resourceMode=true;this._resourceTeam();this.render();this._renderResourcePanel();}
  _renderResourcePanel(){if(!this._resourceMode)return;let p=this.root.querySelector('#resourcePlannerPanel');if(!p){p=document.createElement('div');p.id='resourcePlannerPanel';this.root.appendChild(p);}const d=this._resourceData(),admin=this.rwsIsAdmin(),t=this._resourceTeam(),colors=this._resourceColours(),n=v=>Number(v)||0,_si=this._resourceInspect,_se=_si&&this._resourceEntry(_si.lv,_si.zmk);
    const tabs=d.teams.map((q,i)=>`<button class="hbtn resource-team-tab" data-team="${this.esc(q.id)}" style="border-color:${q.id===(t&&t.id)?q.color:'var(--line)'};box-shadow:${q.id===(t&&t.id)?'inset 0 -3px '+q.color:'none'}"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${q.color}"></span> ${this.esc(q.name||('Team '+(i+1)))}</button>`).join('');
    const zs=t?(t.zones||[]):[],tot=zs.reduce((a,x)=>{a.f+=n(x.formwork);a.c+=n(x.concrete);a.t+=n(x.total);a.w+=n(x.workers);return a;},{f:0,c:0,t:0,w:0});
    const input=(x,k,ph)=>admin?`<input class="resource-qty" data-key="${k}" type="number" min="0" step="any" value="${x[k]==null?'':this.esc(x[k])}" placeholder="${ph}" style="width:70px;padding:5px 6px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt);text-align:right">`:`<b>${x[k]==null?'—':this.esc(x[k])}</b>`;
    const rows=zs.map((x,i)=>`<tr data-i="${i}"><td style="padding:7px 5px;text-align:center"><b style="display:inline-grid;place-items:center;width:24px;height:24px;border-radius:50%;background:${t.color};color:#fff">${i+1}</b></td><td style="padding:7px 5px"><b>${this.esc(x.lv)} · ${this.esc(this._resourceZoneLabel(x.lv,x.zmk))}</b></td><td>${input(x,'formwork','0')}</td><td>${input(x,'concrete','0')}</td><td>${input(x,'total','0')}</td><td>${input(x,'workers','0')}</td><td style="white-space:nowrap">${admin?`<button class="resource-up" title="Move earlier" ${i===0?'disabled':''}>↑</button><button class="resource-down" title="Move later" ${i===zs.length-1?'disabled':''}>↓</button><button class="resource-remove" title="Remove" style="color:var(--crit)">✕</button>`:''}</td></tr>`).join('')||`<tr><td colspan="7" style="padding:28px;text-align:center;color:var(--faint)">${admin?'Click Zones on the map in the required work order.':'No Zones assigned to this team.'}</td></tr>`;
    const inspectHtml=_si?(_se?`<div style="border:2px solid ${_se.team.color};border-radius:10px;padding:10px;margin:8px 0;background:color-mix(in srgb,${_se.team.color} 9%,var(--panel))"><div style="display:flex;align-items:center;gap:7px;margin-bottom:8px"><span style="display:grid;place-items:center;width:28px;height:28px;border-radius:50%;background:${_se.team.color};color:#fff;font-weight:900">${_se.item.order}</span><div><b style="font-size:13px">${this.esc(_si.lv)} · ${this.esc(this._resourceZoneLabel(_si.lv,_si.zmk))}</b><div style="font-size:10px;color:${_se.team.color};font-weight:900">${this.esc(_se.team.name)} · Work order ${_se.item.order}</div></div></div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px">${[['Workers',_se.item.workers],['Formwork Qty',_se.item.formwork],['Concrete Qty',_se.item.concrete],['Total Qty',_se.item.total]].map(v=>`<div style="padding:7px;background:var(--panel);border:1px solid var(--line);border-radius:7px"><small style="display:block;color:var(--dim);font-size:8px">${v[0]}</small><b style="font-size:14px">${v[1]==null?'—':this.esc(v[1])}</b></div>`).join('')}</div></div>`:`<div style="border:1px dashed var(--line);border-radius:9px;padding:10px;margin:8px 0"><b>${this.esc(_si.lv)} · ${this.esc(this._resourceZoneLabel(_si.lv,_si.zmk))}</b><div style="font-size:10px;color:var(--faint)">Not assigned to a team.</div></div>`):'';
    p.style.cssText='position:fixed;right:12px;top:70px;width:min(720px,calc(100vw - 90px));max-height:calc(100vh - 84px);overflow:auto;z-index:190;background:var(--panel);border:1px solid var(--line);border-top:5px solid '+(t?t.color:'#3157d5')+';border-radius:12px;box-shadow:0 18px 55px rgba(0,0,0,.28);padding:12px';
    p.innerHTML=`<div style="display:flex;align-items:center;gap:8px"><div><b style="font-size:18px">🧰 Resource Planning</b><div style="font-size:10px;color:var(--dim)">${admin?'Click an unassigned Zone to add it; click an assigned Zone to view its resources.':'Click any coloured Zone to view its team and resources.'}</div></div><span style="flex:1"></span>${admin?'<button class="hbtn" id="resourceAddTeam">+ Team</button>':''}<button class="hbtn" id="resourceClose">Close ✕</button></div><div style="display:flex;gap:6px;flex-wrap:wrap;margin:12px 0">${tabs}</div>${inspectHtml}${t?`<div style="display:flex;gap:8px;align-items:center;padding:8px;background:var(--panel2);border-radius:8px;margin-bottom:8px">${admin?`<input id="resourceTeamColor" type="color" value="${t.color}" title="Team colour" style="width:30px;height:30px;padding:1px;border:1px solid var(--line);border-radius:6px;background:var(--panel)"><input id="resourceTeamName" value="${this.esc(t.name)}" style="font-weight:900;padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt)">`:`<span style="width:12px;height:12px;border-radius:50%;background:${t.color}"></span><b>${this.esc(t.name)}</b>`}<span style="flex:1"></span><span style="font-size:10px;color:var(--dim)">${zs.length} Zones</span>${admin&&d.teams.length>1?'<button id="resourceDeleteTeam" style="border:0;background:transparent;color:var(--crit);cursor:pointer">Delete team</button>':''}</div><div style="overflow:auto"><table style="width:100%;min-width:650px;border-collapse:collapse;font-size:11px"><thead><tr style="background:var(--panel2);color:var(--dim)"><th style="padding:6px">Order</th><th style="text-align:left">Zone</th><th>Formwork Qty</th><th>Concrete Qty</th><th>Total Qty</th><th>Workers</th><th></th></tr></thead><tbody>${rows}</tbody><tfoot><tr style="background:var(--panel2);font-weight:900"><td colspan="2" style="padding:8px;text-align:right">TEAM TOTAL</td><td style="text-align:right;padding:8px">${this.fmt(tot.f)}</td><td style="text-align:right;padding:8px">${this.fmt(tot.c)}</td><td style="text-align:right;padding:8px">${this.fmt(tot.t)}</td><td style="text-align:right;padding:8px">${this.fmt(tot.w)}</td><td></td></tr></tfoot></table></div>`:'<div class="empty">No teams yet.</div>'}`;
    p.style.display='block';p.querySelector('#resourceClose').onclick=()=>{this._resourceMode=false;this._resourceInspect=null;p.style.display='none';this.render();};p.querySelectorAll('.resource-team-tab').forEach(b=>b.onclick=()=>{this._resourceTeamId=b.dataset.team;this.render();this._renderResourcePanel();});const add=p.querySelector('#resourceAddTeam');if(add)add.onclick=()=>{const name=window.prompt('Team name','Team '+String.fromCharCode(65+d.teams.length));if(!name||!name.trim())return;const id='team_'+Date.now().toString(36);d.teams.push({id,name:name.trim(),color:colors[d.teams.length%colors.length],zones:[]});this._resourceTeamId=id;this._resourceSave();this.render();this._renderResourcePanel();};if(!t)return;const tn=p.querySelector('#resourceTeamName'),tc=p.querySelector('#resourceTeamColor');if(tn)tn.onchange=()=>{t.name=tn.value.trim()||t.name;this._resourceSave();this.render();this._renderResourcePanel();};if(tc)tc.onchange=()=>{t.color=tc.value;this._resourceSave();this.render();this._renderResourcePanel();};const del=p.querySelector('#resourceDeleteTeam');if(del)del.onclick=()=>this._confirmModal('Delete this team and its Zone assignment?',()=>{d.teams=d.teams.filter(q=>q.id!==t.id);this._resourceTeamId=(d.teams[0]||{}).id;this._resourceSave();this.render();this._renderResourcePanel();});p.querySelectorAll('tr[data-i]').forEach(tr=>{const i=+tr.dataset.i;tr.style.cursor='pointer';tr.onclick=e=>{if(e.target.closest('input,button'))return;this._resourceInspect={lv:zs[i].lv,zmk:zs[i].zmk};this.render();this._renderResourcePanel();};tr.querySelectorAll('.resource-qty').forEach(inp=>inp.onchange=()=>{zs[i][inp.dataset.key]=this._resourceNum(inp.value);this._resourceSave();this._resourceInspect={lv:zs[i].lv,zmk:zs[i].zmk};this.render();this._renderResourcePanel();});const up=tr.querySelector('.resource-up'),dn=tr.querySelector('.resource-down'),rm=tr.querySelector('.resource-remove');if(up)up.onclick=()=>{if(i<1)return;[zs[i-1],zs[i]]=[zs[i],zs[i-1]];this._resourceRenumber(t);this._resourceSave();this.render();this._renderResourcePanel();};if(dn)dn.onclick=()=>{if(i>=zs.length-1)return;[zs[i+1],zs[i]]=[zs[i],zs[i+1]];this._resourceRenumber(t);this._resourceSave();this.render();this._renderResourcePanel();};if(rm)rm.onclick=()=>{const gone=zs[i];zs.splice(i,1);if(this._resourceInspect&&this._resourceInspect.lv===gone.lv&&this._resourceInspect.zmk===gone.zmk)this._resourceInspect=null;this._resourceRenumber(t);this._resourceSave();this.render();this._renderResourcePanel();};});}
  ...
  async rwsRenderAdmin(){
    ...
    const AL={EB:'Existing Basement',NB:'New Basement',MA:'Marine',RWS:'RWS · all Reports (view only)',REPEDIT:'Report Edit',PLAN:'Planning Comments',CMT:'Comment (main + M28)',M28_OTHER:'M28 · L2/L3/L4/Ramp/TST',RESOURCE:'Resource Planning (view)',HIST:'History (snapshots)'};
  ...
  drawLegend(){
    ...
    if(this._resourceMode){
      const ts=this._resourceData().teams;s+='<div style="font-weight:800;color:var(--txt);margin-bottom:3px">Resource · Team Legend</div><div style="font-size:9px;color:var(--faint);margin-bottom:5px">Zone colour = assigned team · number = work order</div>';
