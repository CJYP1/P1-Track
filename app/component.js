...
class Component extends DCLogic {
  ...
  _exportCombinedMonthlyPng(cat,lv,mon){
    ...
    x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle=meta.color;x.fillRect(0,0,W,80);x.fillStyle='#fff';x.font='700 27px Arial';x.fillText('P1 Waterfront · '+meta.name+' · '+lv,34,35);x.font='700 16px Arial';x.fillText(mon+' · Target Only versus Actual',34,62);const cols=[34,430,680,875,1035,1200];let y=108;x.font='700 13px Arial';x.fillStyle='#475467';['Zone',mon+' Target Only',mon+' Actual','Achieved','Outstanding','Result'].forEach((h,i)=>x.fillText(h,cols[i],y));y+=20;x.strokeStyle='#cfd7e2';x.beginPath();x.moveTo(28,y);x.lineTo(W-28,y);x.stroke();
    acts.forEach(aid=>{const g=by[aid];if(y+headH>H)return;x.fillStyle='#e8edf3';x.fillRect(28,y,W-56,headH);x.fillStyle='#202938';x.font='700 16px Arial';x.fillText(g.name,40,y+25);y+=headH;g.rows.sort((a,b)=>(b.complete-a.complete)||String(a.zone).localeCompare(String(b.zone))).forEach(r=>{if(y+rowH>H)return;x.fillStyle=r.complete?'#edf8f1':'#fff';x.fillRect(28,y,W-56,rowH);x.fillStyle='#202938';x.font='700 13px Arial';x.fillText(r.zone+(r.critical?'  CRITICAL':''),cols[0],y+25);x.font='13px Arial';x.fillStyle=r.completedEarlier?'#167846':'#202938';x.fillText(r.completedEarlier?'Completed earlier':fmt(r.plan)+(r.unit?' '+r.unit:''),cols[1],y+25);x.fillStyle=r.currentDone>0||r.completedEarlier?'#167846':'#202938';x.fillText(r.completedEarlier?fmt(r.cumDone)+(r.unit?' '+r.unit:'')+' cumulative complete':fmt(r.currentDone)+(r.unit?' '+r.unit:''),cols[2],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.monthPct+'%',cols[3],y+25);x.fillStyle=r.behind>0?'#c8102e':'#667085';x.fillText(r.behind>0?fmt(r.behind)+(r.unit?' '+r.unit:''):'—',cols[4],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.completedEarlier?'Completed earlier':(r.complete?mon+' target achieved':mon+' target outstanding'),cols[5],y+25);x.strokeStyle='#e4e8ee';x.beginPath();x.moveTo(28,y+rowH);x.lineTo(W-28,y+rowH);x.stroke();y+=rowH;});y+=8;});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0)),cell='padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right';return `<div style="overflow-x:auto"><table style="width:100%;min-width:610px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Target Only</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Actual</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Achieved</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':qty(r.plan,r.unit,false)}</td><td style="${cell};background:${r.currentDone>0?'#edf8f1':'transparent'}">${r.completedEarlier?qty(r.cumDone,r.unit,true):qty(r.currentDone,r.unit,r.currentDone>0)}<small style="display:block;color:#667085;font-size:7px">${r.completedEarlier?'Cumulative completed quantity':this.esc(mon)+' actual only'}</small></td><td style="${cell}"><b style="font-size:11px;color:${r.complete?'#167846':'#c8102e'}">${r.monthPct}%</b></td><td style="${cell}"><b style="color:${r.behind>0?'#c8102e':'#167846'}">${r.behind>0?this.esc(fmt(r.behind)):'—'}</b>${r.behind>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:105px">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':(r.complete?`<b style="color:#167846">${this.esc(mon)} target achieved</b>`:`<b style="color:#c8102e">${this.esc(mon)} target outstanding</b>`)}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
    ...
    [...ov.querySelectorAll('div,span')].forEach(el=>{const t=el.textContent.trim();if(!el.querySelector('div,span')&&t.startsWith('✓ Complete · overall scope done'))el.textContent='✓ Complete / '+mon+' Target achieved';else if(!el.querySelector('div,span')&&t==='No work meets both completion conditions yet.')el.textContent='No completed or achieved work yet.';else if(!el.querySelector('div,span')&&t.startsWith('● In progress / monthly outstanding'))el.textContent='● '+mon+' Target outstanding';else if(!el.querySelector('div,span')&&t.startsWith('How this report decides “Complete”'))el.innerHTML=`<b>${this.esc(mon)} Target Only:</b> compares ${this.esc(mon)} Plan with ${this.esc(mon)} Actual only. Previous backlog is not added to Target, Outstanding or Achieved. Activities genuinely completed earlier remain under <b style="color:#167846">Complete</b>.`;});
