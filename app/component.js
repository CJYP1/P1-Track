...
class Component extends DCLogic {
  ...
  _renderCombinedReport(ov,cats,close){
    const defs=this._reportDefs(),areaMeta={EB:{name:'EB · Existing Basement',color:'#d94d86'},NB:{name:'NB · New Basement',color:'#e58b2f'},MA:{name:'MR · Marine',color:'#367bc8'}},fmt=n=>Math.round(Number(n)||0).toLocaleString();
    const statusRow=(cat,r)=>{const liveA=r.actual||this._catchupActual(r.levels,r.aid,cat,r.filter),liveP=r.plan||this._catchupPlan(r.levels,r.aid,cat,r.filter),liveTotal=this._reportCommonTotal(r.levels,r.aid,cat,r.filter,liveA,liveP),ev=this._reportEditedValues(cat,r,liveA,liveP,liveTotal),A=ev.A,P=ev.P,unit=r.unit||'',pct=A.total>0?Math.min(100,Math.round(A.done/A.total*100)):0;return {name:r.a,pct,done:A.done,total:A.total,plan:P.planned,unit,complete:A.total>0&&A.done>=A.total};};
    const listHtml=(rows,complete)=>{const list=rows.filter(r=>r.complete===complete);if(!list.length)return '<div style="padding:12px 9px;color:var(--faint);font-size:10px;text-align:center">None</div>';return list.map(r=>`<div style="padding:8px 9px;border-bottom:1px solid var(--line);background:var(--panel)"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px"><b style="font-size:10.5px;line-height:1.25">${this.esc(r.name)}</b><b style="font-size:11px;color:${complete?'#218a5c':'#b75d13'}">${r.pct}%</b></div><div style="font-size:9px;color:var(--dim);margin-top:4px">${this.esc(fmt(r.done)+' / '+fmt(r.total)+(r.unit?' '+r.unit:''))}${!complete&&r.plan?this.esc(' · plan due '+fmt(r.plan)):''}</div></div>`).join('');};
    const areaCards=cats.map(cat=>{const d=defs[cat],meta=areaMeta[cat],levels=(d&&d.levels)||[],levelBlocks=levels.map(lv=>{const rows=this._liveReportRows(cat,[lv]).map(r=>statusRow(cat,r)),done=rows.filter(r=>r.complete),wip=rows.filter(r=>!r.complete);return `<section style="border:1px solid var(--line);border-radius:9px;overflow:hidden;margin-bottom:10px"><div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:var(--panel2);border-bottom:1px solid var(--line)"><b style="font-size:12px">${this.esc(lv)}</b><span style="font-size:9px;color:var(--dim)">${done.length} completed · ${wip.length} in progress</span></div><div style="display:grid;grid-template-columns:1fr 1fr"><div style="min-width:0;border-right:1px solid var(--line)"><div style="padding:6px 9px;background:#e9f6ee;color:#218a5c;font-size:9px;font-weight:900;text-transform:uppercase">Completed</div>${listHtml(rows,true)}</div><div style="min-width:0"><div style="padding:6px 9px;background:#fff1e4;color:#b75d13;font-size:9px;font-weight:900;text-transform:uppercase">In Progress</div>${listHtml(rows,false)}</div></div></section>`;}).join('');return `<article style="min-width:0;border-top:5px solid ${meta.color};background:var(--panel);border-radius:10px;box-shadow:0 2px 10px rgba(0,0,0,.08);padding:13px"><div style="font-size:15px;font-weight:900;color:${meta.color};margin-bottom:11px">${this.esc(meta.name)}</div>${levelBlocks}</article>`;}).join('');
    ov.innerHTML=`<div style="max-width:1500px;margin:0 auto"><div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:14px"><div style="border-left:6px solid #6d1327;padding-left:14px;flex:1"><div style="font-size:23px;font-weight:800;line-height:1.2"><span style="color:#161616">P1 Waterfront</span> <span style="color:#6d1327">| Combined Status Report</span></div><div style="font-size:11px;color:var(--dim);margin-top:5px">Three main areas · completed and in-progress activities grouped by level</div></div><button class="hbtn" id="laClose">Close ✕</button></div><div class="seg" id="rptViewSeg" style="margin:0 0 16px"><button data-view="area">Area Report</button><button data-view="combined" class="on">Combined Status</button></div><div style="display:grid;grid-template-columns:repeat(${Math.min(3,Math.max(1,cats.length))},minmax(350px,1fr));gap:15px;align-items:start;overflow-x:auto;padding-bottom:18px">${areaCards}</div></div>`;
    ov.style.display='block';const cl=ov.querySelector('#laClose');if(cl)cl.onclick=close;const areaBtn=ov.querySelector('[data-view="area"]');if(areaBtn)areaBtn.onclick=()=>{this._reportCombined=false;this.openLookAhead();};
  }
  openLookAhead(){
