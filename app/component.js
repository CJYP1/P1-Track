...
class Component extends DCLogic {
  ...
  _resourceCoreRenumber(t){(t.cores||[]).sort((a,b)=>(Number(a.order)||999)-(Number(b.order)||999)).forEach((x,i)=>x.order=i+1);}
  ...
  _resourceRenumber(t){(t.zones||[]).sort((a,b)=>(Number(a.order)||999)-(Number(b.order)||999)).forEach((x,i)=>x.order=i+1);}
  ...
  render(){
    ...
    L.zones.forEach((z,i)=>{
      ...
      if(this._resourceMode)op=_monthEntry?0.68:(this._resourceEditing?0.16:0);
      let planst='';
      if(vis&&this.colorMode==='plan'){ const st=this._zoneMonthState(this.curLevel,z,this.planMonth()); op=st.colored?0.62:0.92; }
      if(vis&&this.colorMode==='castdate'){ if(!this._castSlabsOn()){op=0.28;} else {const _ci=this._zoneCastInfo(this.curLevel,z); if(_ci.done)op=0.5;} }   /* 浇筑完成的板: 半透明黑, 跟按月上色的板区分 */
    ...
    _lbls.forEach(lb=>{
      ...
      if(this._resourceMode){const _re=this._resourceEntry(this.curLevel,z.mk||z.lid);if(_re){const _rr=Math.max(780,fs*0.58),_rx=cx,_ry=cy-fs*1.18,_rc=_re.team.color||'#3157d5',_rn=_re.item.order||((_re.team.zones||[]).indexOf(_re.item)+1),_rv=k=>_re.item[k]==null?'—':this.fmt(_re.item[k]),_rw=_rv('workers'),_rf=_rv('formwork'),_rq=_rv('concrete'),_rt=_rv('total');_topDates+=`<g style="pointer-events:none"><title>${this.esc(_re.team.name)} · Work order ${_rn} · Workers ${this.esc(_rw)} · Formwork ${this.esc(_rf)} · Concrete ${this.esc(_rq)} · Total ${this.esc(_rt)}</title><circle cx="${_rx.toFixed(0)}" cy="${_ry.toFixed(0)}" r="${_rr.toFixed(0)}" fill="${_rc}" stroke="#fff" stroke-width="${Math.max(220,_rr*0.18).toFixed(0)}"/><text x="${_rx.toFixed(0)}" y="${(_ry+_rr*0.38).toFixed(0)}" text-anchor="middle" font-size="${(_rr*1.12).toFixed(0)}px" fill="#fff" style="font-weight:900;paint-order:stroke;stroke:${_rc};stroke-width:80px">${_rn}</text><text x="${_rx.toFixed(0)}" y="${(_ry+_rr*1.50).toFixed(0)}" text-anchor="middle" font-size="${Math.max(520,_rr*0.52).toFixed(0)}px" fill="${_rc}" style="font-weight:900;paint-order:stroke;stroke:#fff;stroke-width:180px">${this.esc(_re.team.name)}</text><text x="${_rx.toFixed(0)}" y="${(_ry+_rr*2.28).toFixed(0)}" text-anchor="middle" font-size="${Math.max(720,_rr*0.68).toFixed(0)}px" fill="#202938" style="font-weight:950;paint-order:stroke;stroke:#fff;stroke-width:230px">W: ${this.esc(_rw)}</text><text x="${_rx.toFixed(0)}" y="${(_ry+_rr*3.08).toFixed(0)}" text-anchor="middle" font-size="${Math.max(650,_rr*0.60).toFixed(0)}px" fill="#202938" style="font-weight:900;paint-order:stroke;stroke:#fff;stroke-width:220px">F: ${this.esc(_rf)} · C: ${this.esc(_rq)} · T: ${this.esc(_rt)}</text></g>`;}}
    ...
    if(_subL){
      ...
      if(_resSub){const _rn=_resSub.item.order||((_resSub.team.zones||[]).indexOf(_resSub.item)+1),_rv=k=>_resSub.item[k]==null?'—':this.fmt(_resSub.item[k]),_rw=_rv('workers'),_rf=_rv('formwork'),_rq=_rv('concrete'),_rt=_rv('total'),_rr=Math.max(720,Math.min(1150,_bh*0.09));_topDates+=`<g style="pointer-events:none"><title>${this.esc(_resSub.team.name)} · W ${this.esc(_rw)} · F ${this.esc(_rf)} · C ${this.esc(_rq)} · T ${this.esc(_rt)}</title><circle cx="${lq[0].toFixed(0)}" cy="${(lq[1]-_rr*1.65).toFixed(0)}" r="${_rr.toFixed(0)}" fill="${_resSub.team.color}" stroke="#fff" stroke-width="220"/><text x="${lq[0].toFixed(0)}" y="${(lq[1]-_rr*1.30).toFixed(0)}" text-anchor="middle" font-size="${(_rr*1.08).toFixed(0)}" fill="#fff" font-weight="900">${_rn}</text><text x="${lq[0].toFixed(0)}" y="${(lq[1]+_rr*1.20).toFixed(0)}" text-anchor="middle" font-size="${Math.max(650,_rr*0.62).toFixed(0)}" fill="#202938" stroke="#fff" stroke-width="225" paint-order="stroke" font-weight="950">${this.esc(_resSub.team.name)} · W: ${this.esc(_rw)}</text><text x="${lq[0].toFixed(0)}" y="${(lq[1]+_rr*2.05).toFixed(0)}" text-anchor="middle" font-size="${Math.max(600,_rr*0.56).toFixed(0)}" fill="#202938" stroke="#fff" stroke-width="210" paint-order="stroke" font-weight="900">F: ${this.esc(_rf)} · C: ${this.esc(_rq)} · T: ${this.esc(_rt)}</text></g>`;}
  ...
  buildMetrics(){
    ...
    /* "This month" is the planned-month view with everything that is not working this month
       hidden — a tab of its own so it is one click away instead of two. */
    const modes=[['area','Area',(this.CAT.NB).c],['plan','Planned (month)',this.cssvar('--accent')],['month','This month','#0e9f6e']];if(this.rwsCanResource())modes.push(['resource','Resource','#3157d5']);
