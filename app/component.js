...
class Component extends DCLogic {
  ...
  addActCmt(lv,zmk,aid,text,to){text=(text||'').trim();if(!text)return;to=String(to||'');if(to!=='PM'&&to!=='Planning'){this._toast&&this._toast('Please choose To PM or To Planning');return;}if(!(this.rwsIsAdmin()||this.rwsCanComment(lv,zmk))){this.rwsDeny('You cannot comment on this area.');return;}const s=(typeof rwsGetSession==='function'&&rwsGetSession())||{};const ts=new Date().toISOString();const k=lv+'||'+zmk+'||'+aid+'||'+ts+'_'+Math.random().toString(36).slice(2,6);const v={t:text,to,by:s.username||'user',ts};this._actCmt=this._actCmt||{};this._actCmt[k]=v;this.saveActCmt();if(typeof rwsSyncKV==='function')rwsSyncKV('act_cmt',k,v,lv,zmk);this._actRerender(this._selZone());}
  ...
  _levelOpenCmts(lv){const pre=lv+'||',o=this._actCmt||{};return Object.keys(o).filter(k=>k.indexOf(pre)===0&&o[k]&&!o[k].done).map(k=>{const p=k.split('||'),zmk=p[1],aid=p[2],z=((this.DATA.levels[lv]||{}).zones||[]).find(x=>this.zid(x)===zmk),am=this._actMeta().find(a=>a.id===aid);return {k,...o[k],lv,zmk,aid,zone:z?z.label:(zmk.indexOf(lv+'|')===0?zmk.slice(lv.length+1):zmk),activity:am?am.label:aid};}).sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.activity).localeCompare(String(b.activity))||String(a.ts||'').localeCompare(String(b.ts||'')));}
  ...
  _openLevelCmt(k){const p=String(k||'').split('||'),lv=p[0],zmk=p[1];if(this._zoneFindZone(lv,zmk)){this.gotoZone(lv,zmk);return;}if(lv==='L1'&&zmk.indexOf('L1|')===0){const lab=zmk.slice(3);for(const kind of ['P','C']){const i=((this.SUBZONES.L1||{})[kind]||[]).findIndex(x=>String(x.label)===lab);if(i>=0){if(lv!==this.curLevel)this.setLevel(lv);this.selectSubzone(kind,i);return;}}}}
  ...
  openLookAhead(){
    ...
    const cat=this._reportCat, def=defs[cat],admin=this.rwsIsAdmin(),editing=admin&&!!this._reportEditing;
    ...
    const makeRows=(reportRows)=>reportRows.map(r=>{ const liveA=r.actual||this._catchupActual(r.levels,r.aid,cat,r.filter),liveP=r.plan||this._catchupPlan(r.levels,r.aid,cat,r.filter),liveTotal=this._reportCommonTotal(r.levels,r.aid,cat,r.filter,liveA,liveP),ev=this._reportEditedValues(cat,r,liveA,liveP,liveTotal),A=ev.A,P=ev.P,den=A.total,tgt=den>0?Math.min(100,Math.round(P.planned/den*100)):0,unit=r.unit?(' '+r.unit):''; const actSub=`(${fmtN(A.done)}/${fmtN(den)}${unit})`,planSub=`(${fmtN(P.planned)}/${fmtN(den)}${unit} planned to date)`,by=P.end?this._fmtDShort(P.end):'—',inp='width:70px;padding:5px 6px;border:1px solid #cbaeb4;border-radius:5px;text-align:center;font:700 12px Segoe UI;background:#fff;color:#2b1114';
      ...
      return `<tr data-rkey="${this.esc(ev.key)}">`
        +`<td style="background:#6d1327;color:#fff;font-weight:800;padding:11px 9px;font-size:11.5px;vertical-align:middle;width:27%">${this.esc(r.a)}${editing?`<button type="button" class="rpt-live" style="display:block;margin-top:8px;padding:3px 7px;border:1px solid rgba(255,255,255,.65);border-radius:5px;background:${ev.has?'#fff':'transparent'};color:${ev.has?'#6d1327':'#fff'};font-size:8.5px;cursor:pointer">↺ Use live</button>`:''}</td>`
    ...
    const sb=ov.querySelector('#rptSave');if(sb)sb.onclick=()=>{const all={...((this._appCfg&&this._appCfg.reportOverrides)||{})};let bad=null;ov.querySelectorAll('tr[data-rkey]').forEach(tr=>{if(bad)return;const vals={},live={};tr.querySelectorAll('.rpt-edit').forEach(i=>{const raw=i.value.trim(),n=Number(raw);if(raw===''||!Number.isInteger(n)||n<0){bad=i;return;}vals[i.dataset.field]=n;live[i.dataset.field]=Math.max(0,Math.round(Number(i.dataset.live)||0));});if(bad)return;const key=tr.dataset.rkey;if(tr.dataset.useLive==='1'||(vals.planned===live.planned&&vals.total===live.total&&vals.done===live.done))delete all[key];else all[key]={planned:Math.min(vals.total,vals.planned),done:Math.min(vals.total,vals.done),total:vals.total};});if(bad){bad.style.borderColor='#c8102e';bad.focus();this._toast('Report values must be whole numbers.');return;}this._appCfg=this._appCfg||{};this._appCfg.reportOverrides=all;try{localStorage.setItem('rws_app_cfg',JSON.stringify(this._appCfg));}catch(e){}try{if(typeof rwsSyncKV==='function')Promise.resolve(rwsSyncKV('settings','reportOverrides',all,null,null)).catch(()=>{});}catch(e){}this._reportEditing=false;this._toast('Report updated.');this.openLookAhead();}; }
  zoneFill(z){
