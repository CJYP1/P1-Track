...
class Component extends DCLogic {
  ...
  render(){
    ...
    const _resTeams={};
    const _resMark=(t,x,y)=>{if(!t)return;const k=t.id||t.name;const e=_resTeams[k]||(_resTeams[k]={t,xs:[],ys:[]});e.xs.push(x);e.ys.push(y);};
    ...
    if(this._resourceMode){   /* one headline number per Team, centred on that Team's zones */
      const _bw=Math.max(this.vb.w,1),_bs=_bw*0.055;   /* scales with the zoom so it stays readable */
      Object.keys(_resTeams).forEach(k=>{const e=_resTeams[k];if(!e.xs.length)return;
        const x=e.xs.reduce((a,b)=>a+b,0)/e.xs.length,y=e.ys.reduce((a,b)=>a+b,0)/e.ys.length;
        ...
        _topDates+=`<g style="pointer-events:none">`
          +`<text x="${x.toFixed(0)}" y="${y.toFixed(0)}" text-anchor="middle" font-size="${_bs.toFixed(0)}px" fill="${c}" style="font-weight:950;paint-order:stroke;stroke:#fff;stroke-width:${(_bs*0.20).toFixed(0)}px">${this.fmt(w)}</text>`
          +`<text x="${x.toFixed(0)}" y="${(y+_bs*0.42).toFixed(0)}" text-anchor="middle" font-size="${(_bs*0.26).toFixed(0)}px" fill="${c}" style="font-weight:900;letter-spacing:0.08em;paint-order:stroke;stroke:#fff;stroke-width:${(_bs*0.09).toFixed(0)}px">${this.esc(String(e.t.name||'').toUpperCase())} · WORKERS</text>`
          +`</g>`;});
    }
