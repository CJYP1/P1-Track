...
class Component extends DCLogic {
  ...
  _canSeeActCmt(c,lv,zmk){if(this.rwsIsAdmin())return true;if(!c||!this._rwsUser)return false;if(c.to==='Planning')return this.rwsIsPlanning();if(c.to==='PM')return this.rwsScopeOk(lv,zmk);return true;}   // 旧的 To Unassigned 评论继续让所有已登录账号看到
  _canResolveActCmt(c,lv,zmk){if(this.rwsIsAdmin())return true;if(!c||!c.to)return false;if(c.to==='Planning')return this.rwsIsPlanning();if(c.to==='PM')return this.rwsScopeOk(lv,zmk);return false;}
  ...
  actPourPct(lv,zmk,aid){const v=this.actRes(lv,zmk,aid,'pour');if(v==null)return null;const n=Math.max(0,Math.min(100,Math.round(+v||0)));return n<=0?0:(n<=33?33:(n<=67?67:100));}
  ...
  zpSection(z){
    ...
    const rows=acts.filter(a=>a.custom||this._actApplies(a.id,lv,z))
      ...
      const _pourLabel=_pourPct==null?'Not entered':(_pourPct===0?'0/3 pours · 0%':(_pourPct===33?'1/3 pours · 33%':(_pourPct===67?'2/3 pours · 67%':'3/3 pours · 100%')));
      const pourRow=_isPourAct?`<div class="actsub actpour"><span class="am2">Concrete pouring</span>${canEdit?`<select class="act-pour-in" data-a="${a.id}" title="Three-stage concrete pour progress"><option value=""${_pourPct==null?' selected':''}>Not entered</option><option value="0"${_pourPct===0?' selected':''}>0/3 · 0%</option><option value="33"${_pourPct===33?' selected':''}>1/3 · 33%</option><option value="67"${_pourPct===67?' selected':''}>2/3 · 67%</option><option value="100"${_pourPct===100?' selected':''}>3/3 · 100%</option></select>`:`<b>${_pourLabel}</b>`}<span class="pour-note">3 pours</span></div>`:'';
  ...
  selectZone(z,sub){
    ...
    {const _zk=[this.curLevel,z.mk||z.lid];
     ...
     sb.querySelectorAll('.act-pour-in').forEach(el=>el.addEventListener('change',()=>{this.setActRes(_zk[0],_zk[1],el.dataset.a,'pour',el.value);this._actRerender(z);}));
