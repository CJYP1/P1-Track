...
class Component extends DCLogic {
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const levelRows=(cat,lv)=>{const rows=[];this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).filter(a=>this._reportStructureAid(a.id)).forEach(a=>{const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0);if(p<=0&&d<=0)return;const carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null;let cumPlan=0,cumDone=0;for(let j=0;j<=mi;j++){cumPlan+=Number(this.actPlan(lv,zmk,a.id,this.ACT_MONTHS[j])||0);cumDone+=Number(this.actDoneMonth(lv,zmk,a.id,this.ACT_MONTHS[j])||0);}const behind=carry&&carry.balance>0?carry.balance:0,complete=behind<=0&&(cumPlan>0||cumDone>0),pct=cumPlan>0?Math.min(100,Math.round(cumDone/cumPlan*100)):(cumDone>0?100:0);rows.push({lv,zmk,zone,name:a.label,plan:p,done:d,cumPlan,cumDone,unit:a.unit||'',pct,complete,critical:!!z.crit,behind});});});return rows.sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.name).localeCompare(String(b.name)));};
    const listHtml=(rows,complete)=>{const list=rows.filter(r=>r.complete===complete);if(!list.length)return '<div style="padding:16px 10px;color:#98a2b3;background:#fff;font-size:10px;text-align:center">None this month</div>';return list.map(r=>`<div style="padding:9px 10px;border-bottom:1px solid #e1e6ed;background:#fff;color:#202938"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px"><div style="min-width:0"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-size:11px;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<span style="margin-left:5px;color:#c8102e;font-size:8px;font-weight:900">CRITICAL</span>':''}<div style="font-size:9.5px;color:#667085;margin-top:2px">${this.esc(r.name)}</div></div><div style="text-align:right;white-space:nowrap" title="Cumulative actual divided by cumulative plan through ${this.esc(mon)}"><b style="display:block;font-size:11px;color:${complete?'#218a5c':'#b75d13'}">${r.pct}%</b><span style="font-size:8px;color:#98a2b3">Cum. ${this.esc(fmt(r.cumDone))}/${this.esc(fmt(r.cumPlan))}</span></div></div><div style="font-size:9px;color:#667085;margin-top:5px">This month: Plan <b>${this.esc(fmt(r.plan))}</b>${r.unit?' '+this.esc(r.unit):''} · Actual <b style="color:${complete?'#218a5c':'#b75d13'}">${this.esc(fmt(r.done))}</b>${r.unit?' '+this.esc(r.unit):''}${r.behind>0?` · <b style="color:#c8102e">Behind ${this.esc(fmt(r.behind))}</b>`:' · <b style="color:#218a5c">On track</b>'}</div></div>`).join('');};
    const areaCards=visibleCats.map(cat=>{const d=defs[cat],meta=areaMeta[cat],levels=(d&&d.levels)||[],levelBlocks=levels.map(lv=>{const rows=levelRows(cat,lv),done=rows.filter(r=>r.complete),wip=rows.filter(r=>!r.complete);return `<section style="min-width:360px;border:1px solid #d9e0e8;border-radius:8px;overflow:hidden;background:#fff;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#eef1f5;border-bottom:1px solid #d9e0e8"><b style="font-size:14px;color:#202938">${this.esc(lv)}</b><span style="font-size:9.5px;color:#667085;font-weight:700">${done.length} completed · ${wip.length} in progress</span></div><div style="display:grid;grid-template-columns:1fr 1fr"><div style="min-width:0;border-right:1px solid #d9e0e8"><div style="padding:8px 10px;background:#dff3e7;color:#167846;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.35px">✓ Completed</div>${listHtml(rows,true)}</div><div style="min-width:0"><div style="padding:8px 10px;background:#ffead7;color:#a74d09;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.35px">● In Progress</div>${listHtml(rows,false)}</div></div></section>`;}).join('');return `<article style="min-width:0;border:1px solid #d9e0e8;background:#fff;border-radius:10px;box-shadow:0 3px 14px rgba(24,39,75,.09);overflow:hidden;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;padding:13px 17px;background:${meta.color};color:#fff"><div style="font-size:18px;font-weight:900">${this.esc(meta.name)}</div><div style="font-size:12px;font-weight:900;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.45);border-radius:20px;padding:4px 11px">${this.esc(mon)}</div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(460px,1fr));gap:14px;align-items:start;padding:16px">${levelBlocks}</div></article>`;}).join('');
