...
class Component extends DCLogic {
  ...
  _reportZones(lv,cat){const L=this.DATA.levels[lv],out=(L&&L.zones||[]).filter(z=>(z.cat||'NB')===cat);if(cat==='MA'&&lv==='L1'){const S=this.SUBZONES&&this.SUBZONES.L1;['C','P'].forEach(k=>(S&&S[k]||[]).forEach(e=>out.push({mk:'L1|'+e.label,label:e.label,cat:'MA',area:e.a||0,cols:k==='P'?((this._marineCol&&this._marineCol[e.label])||[]):[],piles:[],beams:[],lifts:[],stairs:[],cores:[],counts:{},_mslab:k==='C',_pod:k==='P'})));}return out;}
  _catchupPlan(levels,aid,cat,filter){const asOf=this._reportToday();let planned=0,total=0,end=null;
    (levels||[]).forEach(lv=>{this._reportZones(lv,cat).forEach(z=>{if(!this._reportZoneOk(z,cat,filter))return;const zmk=z.mk||z.lid,d=(this._actDate||{})[lv+'||'+zmk+'||'+aid]||{};let any=false,zt=0,zp=0;
  ...
  openLookAhead(){
    ...
    const makeRows=(reportRows)=>reportRows.map(r=>{ const A=r.actual||this._catchupActual(r.levels,r.aid,cat,r.filter),P=r.plan||this._catchupPlan(r.levels,r.aid,cat,r.filter),den=A.total||P.total,tgt=den>0?Math.min(100,Math.round(P.planned/den*100)):0,unit=r.unit?(' '+r.unit):''; const actSub=`(${fmtN(A.done)}/${fmtN(A.total)}${unit})`,planSub=`(${fmtN(P.planned)}/${fmtN(den)}${unit} planned to date)`,by=P.end?this._fmtDShort(P.end):'—';
      return `<tr>`
        ...
        +`<td style="background:#f4dbdf;color:#2b1114;text-align:center;padding:10px 8px;vertical-align:middle;width:37%"><div style="font-size:13px">${tgt}%</div><div style="font-size:9.5px;color:#6d3b40;margin-top:3px;line-height:1.35">${planSub}<br>Complete by ${this.esc(by)}</div></td>`
        +`<td style="background:#f8e9ec;color:#2b1114;text-align:center;padding:10px 8px;vertical-align:middle;width:36%"><div style="font-size:13px">${A.pct}%</div><div style="font-size:9.5px;color:#6d3b40;margin-top:3px">${actSub}</div></td>`
        +`</tr>`; }).join('');
