...
class Component extends DCLogic {
  ...
  _renderResourcePanel(){if(!this._resourceMode)return;let p=this.root.querySelector('#resourcePlannerPanel');if(!p){p=document.createElement('div');p.id='resourcePlannerPanel';this.root.appendChild(p);}const d=this._resourceData(),admin=this.rwsIsAdmin(),t=this._resourceTeam(),colors=this._resourceColours(),n=v=>Number(v)||0;
  ...
  render(){
    ...
    this.svg.querySelectorAll('.zone').forEach(el=>{
      ...
      el.addEventListener('click',()=>{if(this._placingCol)return;const z=L.zones[+el.dataset.i];if(!this.zoneVisible(z))return;if(this._resourceMode&&this.rwsIsAdmin()){this._resourceToggleZone(this.curLevel,z);return;}if(this.rwsIsAdmin()&&this._adminAggMode){this._adminAggToggle(this.curLevel,z);this.render();}this.selKey=this.zid(z);this.selectZone(z);this.paintSel();this.paintTimelineSel();});
    });
