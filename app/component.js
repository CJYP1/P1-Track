...
class Component extends DCLogic {
  ...
  _resourceTeamValues(t){const saved=(t&&t.resources)||{},sum={workers:0,formwork:0,concrete:0,total:0};(t&&t.zones||[]).forEach(x=>Object.keys(sum).forEach(k=>sum[k]+=Number(x[k])||0));const out={};Object.keys(sum).forEach(k=>out[k]=saved[k]==null?sum[k]:saved[k]);return out;}
  ...
  _renderResourcePanel(){if(!this._resourceMode)return;const side=this.root.querySelector('#side'),title=this.root.querySelector('#sideTitle'),kpis=this.root.querySelector('#kpis'),prog=this.root.querySelector('#progroll'),foot=this.root.querySelector('#foot'),p=this.root.querySelector('#sidebody');if(!side||!p)return;const old=this.root.querySelector('#resourcePlannerPanel');if(old)old.remove();p.id='sidebody';p.dataset.resource='1';if(title)title.textContent='🧰 Resource Planning';if(kpis)kpis.style.display='none';if(prog)prog.style.display='none';if(foot)foot.style.display='none';const d=this._resourceData(),admin=this.rwsIsAdmin(),editing=admin&&!!this._resourceEditing,t=this._resourceTeam(),colors=this._resourceColours(),zs=t?(t.zones||[]).sort((a,b)=>(Number(a.order)||999)-(Number(b.order)||999)):[],tot=t?this._resourceTeamValues(t):{workers:0,formwork:0,concrete:0,total:0},selected=this._resourceInspect&&this._resourceEntry(this._resourceInspect.lv,this._resourceInspect.zmk);
    ...
    const teamResource=t?`<section style="margin:8px 0;padding:10px;border:2px solid ${t.color};border-radius:9px;background:var(--panel2)"><div style="font-size:11px;font-weight:900;color:${t.color};margin-bottom:7px">${this.esc(t.name)} · TEAM TOTAL</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px">${input(tot,'workers','Workers 人数')}${input(tot,'formwork','Formwork 模板量')}${input(tot,'concrete','Concrete 洋灰量')}${input(tot,'total','Total 总量')}</div><div style="font-size:8.5px;color:var(--faint);margin-top:6px">Enter once for the whole Team. Assigned Zones below only show work area and order.</div></section>`:'';
  ...
  drawLegend(){
    ...
    if(this._resourceMode){
      const ts=this._resourceData().teams;s+='<div style="font-weight:800;color:var(--txt);margin-bottom:3px">Resource · Team Legend</div><div style="font-size:9px;color:var(--faint);margin-bottom:5px">Zone colour = Team · number = work order<br>W/F/C/T = whole Team totals</div>';
      ts.forEach(t=>{const n=(t.zones||[]).filter(x=>x.lv===this.curLevel).length,v=this._resourceTeamValues(t);s+=`<div class="lr"><span class="sw" style="background:${t.color}"></span><b>${this.esc(t.name)}</b><span style="margin-left:auto;color:var(--faint)">${n} zone${n===1?'':'s'}</span></div><div style="margin:-2px 0 4px 20px;font-size:8px;color:var(--dim)">W ${this.fmt(v.workers)} · F ${this.fmt(v.formwork)} · C ${this.fmt(v.concrete)} · T ${this.fmt(v.total)}</div>`;});
