...
class Component extends DCLogic {
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{if(!rows.length)return '<div style="padding:22px;color:#98a2b3;background:#fff;font-size:10px;text-align:center">No planned work, actual work or outstanding balance for this month.</div>';return `<div style="overflow-x:auto"><table style="width:100%;min-width:760px;border-collapse:collapse;background:#fff;color:#202938;font-size:10px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:8px 10px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:8px 10px;border-bottom:1px solid #d9e0e8">Activity</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Month Plan</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Month Actual</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Cumulative Actual</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Progress</th><th style="padding:8px 10px;border-bottom:1px solid #d9e0e8">Status</th></tr></thead><tbody>${rows.map(r=>`<tr><td style="padding:8px 10px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7.5px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="padding:8px 10px;border-bottom:1px solid #e7ebf0;font-weight:700">${this.esc(r.name)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.plan,r.unit,false)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right;background:${r.done>0?'#edf8f1':'transparent'}">${qty(r.done,r.unit,r.done>0)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.cumDone,r.unit,false)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right"><b style="color:${r.behind>0?'#b54708':'#167846'}">${r.pct}%</b><small style="display:block;margin-top:1px;color:#98a2b3;font-size:8px">${this.esc(fmt(r.cumDone))}/${this.esc(fmt(r.cumPlan))}</small></td><td style="padding:8px 10px;border-bottom:1px solid #e7ebf0;white-space:nowrap">${r.behind>0?`<b style="color:#c8102e">Behind ${this.esc(fmt(r.behind))}${r.unit?' '+this.esc(r.unit):''}</b>`:(r.done>0?'<b style="color:#167846">Actual entered</b>':'<b style="color:#167846">On track</b>')}</td></tr>`).join('')}</tbody></table></div>`;};
    const areaCards=visibleCats.map(cat=>{const d=defs[cat],meta=areaMeta[cat],levels=(d&&d.levels)||[],levelBlocks=levels.map(lv=>{const rows=levelRows(cat,lv),monthDone=rows.reduce((n,r)=>n+r.done,0);return `<section style="border:1px solid #d9e0e8;border-radius:8px;overflow:hidden;background:#fff;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#e8edf3;border-bottom:1px solid #d9e0e8"><b style="font-size:14px;color:#202938">${this.esc(lv)}</b><span style="font-size:9.5px;color:#667085;font-weight:700">${rows.length} activities · Month actual ${this.esc(fmt(monthDone))}</span></div>${tableHtml(rows)}</section>`;}).join('');return `<article style="min-width:0;border:1px solid #d9e0e8;background:#fff;border-radius:10px;box-shadow:0 3px 14px rgba(24,39,75,.09);overflow:hidden;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;padding:13px 17px;background:${meta.color};color:#fff"><div style="font-size:18px;font-weight:900">${this.esc(meta.name)}</div><div style="font-size:12px;font-weight:900;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.45);border-radius:20px;padding:4px 11px">${this.esc(mon)}</div></div><div style="display:grid;grid-template-columns:1fr;gap:15px;padding:16px">${levelBlocks}</div></article>`;}).join('');
