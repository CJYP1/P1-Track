...
class Component extends DCLogic {
  ...
  _renderCombinedReport(ov,cats,close){
    const defs=this._reportDefs(),mon=(this.ACT_MONTHS||[]).indexOf(this._reportCombinedMonth)>=0?this._reportCombinedMonth:this.actCurLabel(),mi=(this.ACT_MONTHS||[]).indexOf(mon),areaMeta={EB:{name:'EB · Existing Basement',color:'#d94d86'},NB:{name:'NB · New Basement',color:'#e58b2f'},MA:{name:'MR · Marine',color:'#367bc8'}},fmt=n=>Math.round(Number(n)||0).toLocaleString();this._reportCombinedMonth=mon;
    const levelRows=(cat,lv)=>{const rows=[];this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).filter(a=>this._reportStructureAid(a.id)).forEach(a=>{const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0);if(p<=0&&d<=0)return;const complete=p>0?d>=p:d>0,pct=p>0?Math.min(100,Math.round(d/p*100)):(d>0?100:0),carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null;rows.push({lv,zmk,zone,name:a.label,plan:p,done:d,unit:a.unit||'',pct,complete,critical:!!z.crit,behind:carry&&carry.balance>0?carry.balance:0});});});return rows.sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.name).localeCompare(String(b.name)));};
    const listHtml=(rows,complete)=>{const list=rows.filter(r=>r.complete===complete);if(!list.length)return '<div style="padding:16px 10px;color:var(--faint);font-size:10px;text-align:center">None this month</div>';return list.map(r=>`<div style="padding:9px 10px;border-bottom:1px solid var(--line);background:var(--panel)"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px"><div style="min-width:0"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:var(--txt);font:inherit;font-size:11px;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<span style="margin-left:5px;color:#c8102e;font-size:8px;font-weight:900">CRITICAL</span>':''}<div style="font-size:9.5px;color:var(--dim);margin-top:2px">${this.esc(r.name)}</div></div><b style="font-size:11px;color:${complete?'#218a5c':'#b75d13'};white-space:nowrap">${r.pct}%</b></div><div style="font-size:9px;color:var(--dim);margin-top:5px">Plan <b>${this.esc(fmt(r.plan))}</b>${r.unit?' '+this.esc(r.unit):''} · Actual <b style="color:${complete?'#218a5c':'#b75d13'}">${this.esc(fmt(r.done))}</b>${r.unit?' '+this.esc(r.unit):''}${r.behind>0?` · <b style="color:#c8102e">Behind ${this.esc(fmt(r.behind))}</b>`:' · <b style="color:#218a5c">On track</b>'}</div></div>`).join('');};
    const areaCards=cats.map(cat=>{const d=defs[cat],meta=areaMeta[cat],levels=(d&&d.levels)||[],levelBlocks=levels.map(lv=>{const rows=levelRows(cat,lv),done=rows.filter(r=>r.complete),wip=rows.filter(r=>!r.complete);return `<section style="min-width:360px;border:1px solid var(--line);border-radius:9px;overflow:hidden"><div style="display:flex;align-items:center;justify-content:space-between;padding:9px 11px;background:var(--panel2);border-bottom:1px solid var(--line)"><b style="font-size:13px">${this.esc(lv)}</b><span style="font-size:9px;color:var(--dim)">${done.length} completed · ${wip.length} in progress</span></div><div style="display:grid;grid-template-columns:1fr 1fr"><div style="min-width:0;border-right:1px solid var(--line)"><div style="padding:7px 10px;background:#e9f6ee;color:#218a5c;font-size:9px;font-weight:900;text-transform:uppercase">Completed</div>${listHtml(rows,true)}</div><div style="min-width:0"><div style="padding:7px 10px;background:#fff1e4;color:#b75d13;font-size:9px;font-weight:900;text-transform:uppercase">In Progress</div>${listHtml(rows,false)}</div></div></section>`;}).join('');return `<article style="min-width:0;border-top:5px solid ${meta.color};background:var(--panel);border-radius:10px;box-shadow:0 2px 10px rgba(0,0,0,.08);padding:14px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div style="font-size:16px;font-weight:900;color:${meta.color}">${this.esc(meta.name)}</div><div style="font-size:10px;font-weight:800;color:var(--dim)">${this.esc(mon)}</div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(420px,1fr));gap:12px;align-items:start">${levelBlocks}</div></article>`;}).join('');
    const monthBtns=(this.ACT_MONTHS||[]).map(m=>`<button type="button" data-rpt-month="${this.esc(m)}" class="${m===mon?'on':''}">${this.esc(m)}</button>`).join('');
    ov.innerHTML=`<div style="max-width:1500px;margin:0 auto"><div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:14px"><div style="border-left:6px solid #6d1327;padding-left:14px;flex:1"><div style="font-size:23px;font-weight:800;line-height:1.2"><span style="color:#161616">P1 Waterfront</span> <span style="color:#6d1327">| Combined Construction Schedule</span></div><div style="font-size:11px;color:var(--dim);margin-top:5px">${this.esc(mon)} · monthly Plan versus Actual · grouped by area, level and zone</div></div><button class="hbtn" id="laClose">Close ✕</button></div><div class="seg" id="rptViewSeg" style="margin:0 0 11px"><button data-view="area">Area Report</button><button data-view="combined" class="on">Combined Schedule</button></div><div class="seg" id="rptCombinedMonths" style="display:flex;overflow-x:auto;gap:5px;margin:0 0 16px;padding:2px 1px 6px">${monthBtns}</div><div style="display:grid;grid-template-columns:1fr;gap:18px;padding-bottom:18px">${areaCards}</div></div>`;
    ov.style.display='block';const cl=ov.querySelector('#laClose');if(cl)cl.onclick=close;const areaBtn=ov.querySelector('[data-view="area"]');if(areaBtn)areaBtn.onclick=()=>{this._reportCombined=false;this.openLookAhead();};ov.querySelectorAll('[data-rpt-month]').forEach(b=>b.onclick=()=>{this._reportCombinedMonth=b.dataset.rptMonth;this._renderCombinedReport(ov,cats,close);});ov.querySelectorAll('.combined-zone-open').forEach(b=>b.onclick=()=>{const lv=b.dataset.lv,zmk=b.dataset.zmk;close();if(this._zoneFindZone(lv,zmk)){this.gotoZone(lv,zmk);return;}if(lv==='L1'&&zmk.indexOf('L1|')===0){const lab=zmk.slice(3);for(const kind of ['P','C']){const i=((this.SUBZONES.L1||{})[kind]||[]).findIndex(x=>String(x.label)===lab);if(i>=0){if(lv!==this.curLevel)this.setLevel(lv);this.selectSubzone(kind,i);break;}}}});
  }
  openLookAhead(){
