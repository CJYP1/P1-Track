...
class Component extends DCLogic {
  ...
  _exportCombinedMonthlyPng(cat,lv,mon){
    ...
    rows.forEach(r=>(by[r.aid]||(by[r.aid]={name:r.name,rows:[]})).rows.push(r));const acts=Object.keys(by).sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);return (ai<0?999:ai)-(bi<0?999:bi)||by[a].name.localeCompare(by[b].name);}),W=1750,rowH=42,headH=38,H=Math.min(30000,165+acts.length*(headH+8)+rows.length*rowH),cv=document.createElement('canvas');cv.width=W;cv.height=H;const x=cv.getContext('2d'),fmt=n=>Math.round(Number(n)||0).toLocaleString();
    x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle=meta.color;x.fillRect(0,0,W,80);x.fillStyle='#fff';x.font='700 27px Arial';x.fillText('P1 Waterfront · '+meta.name+' · '+lv,34,35);x.font='700 16px Arial';x.fillText(mon+' · Target versus Actual / Early Completion',34,62);const cols=[34,430,650,865,1095,1260,1430];let y=108;x.font='700 13px Arial';x.fillStyle='#475467';['Level · Zone',mon+' Target Only',mon+' Actual / Early Done','Cumulative Actual / Site Progress','Monthly Achieved','Outstanding','Result'].forEach((h,i)=>x.fillText(h,cols[i],y));y+=20;x.strokeStyle='#cfd7e2';x.beginPath();x.moveTo(28,y);x.lineTo(W-28,y);x.stroke();
    acts.forEach(aid=>{const g=by[aid];if(y+headH>H)return;x.fillStyle='#e8edf3';x.fillRect(28,y,W-56,headH);x.fillStyle='#202938';x.font='700 16px Arial';x.fillText(g.name,40,y+25);y+=headH;g.rows.sort((a,b)=>(b.complete-a.complete)||String(a.zone).localeCompare(String(b.zone))).forEach(r=>{if(y+rowH>H)return;x.fillStyle=r.complete?'#edf8f1':'#fff';x.fillRect(28,y,W-56,rowH);x.fillStyle='#202938';x.font='700 13px Arial';x.fillText(r.lv+' · '+r.zone+(r.critical?'  CRITICAL':''),cols[0],y+25);x.font='13px Arial';x.fillText(fmt(r.plan)+(r.unit?' '+r.unit:''),cols[1],y+25);x.fillStyle=r.applied>0?'#167846':'#202938';x.fillText(fmt(r.applied)+(r.unit?' '+r.unit:'')+(r.earlierCredit>0?'  (early '+fmt(r.earlierCredit)+')':''),cols[2],y+25);x.fillStyle=r.sitePct>=100?'#167846':'#5265d5';x.fillText(fmt(r.cumDone)+' / '+fmt(r.scopeTotal)+(r.unit?' '+r.unit:'')+' · '+r.sitePct+'%',cols[3],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.monthPct+'%',cols[4],y+25);x.fillStyle=r.behind>0?'#c8102e':'#667085';x.fillText(r.behind>0?fmt(r.behind)+(r.unit?' '+r.unit:''):'—',cols[5],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.backlogRecovery?(r.complete?'Previous backlog cleared':'Previous backlog remaining'):(r.complete?(r.earlierCredit>0?'Completed early for '+mon:mon+' target achieved'):mon+' target outstanding'),cols[6],y+25);x.strokeStyle='#e4e8ee';x.beginPath();x.moveTo(28,y+rowH);x.lineTo(W-28,y+rowH);x.stroke();y+=rowH;});y+=8;});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{
      ...
      const body=order.map(lv=>`<tr><td colspan="7" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>{
        const monthNote=r.backlogRecovery?`${this.esc(mon)} Actual ${this.esc(fmt(r.currentDone))} · applied to previous backlog`:`${this.esc(mon)} Actual ${this.esc(fmt(r.currentDone))}${r.earlierCredit>0?' + completed early '+this.esc(fmt(r.earlierCredit)):''}`;
        const result=r.backlogRecovery?(r.complete?'Previous backlog cleared':'Previous backlog remaining'):(r.complete?(r.earlierCredit>0?'Completed early for '+mon:mon+' target achieved'):mon+' target outstanding');
        return `<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open ${this.esc(r.lv)} · ${this.esc(r.zoneLabel||r.zone)} details">${this.esc(r.lv)} · ${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${qty(r.plan,r.unit,false)}${r.backlogRecovery?`<small style="display:block;color:#b54708;font-size:7px;font-weight:800">Backlog in ${this.esc(fmt(r.previousBacklog))}</small>`:''}</td><td style="${cell};background:${r.applied>0?'#edf8f1':'transparent'}">${qty(r.applied,r.unit,r.applied>0)}<small style="display:block;color:#667085;font-size:7px">${monthNote}</small></td><td style="${cell};background:#f4f7fb">${qty(r.cumDone,r.unit,r.cumDone>0)}<small style="display:block;color:${r.sitePct>=100?'#167846':'#5265d5'};font-size:8px;font-weight:900">${this.esc(fmt(r.cumDone))} / ${this.esc(fmt(r.scopeTotal))} · ${r.sitePct}%</small></td><td style="${cell}"><b style="font-size:11px;color:${r.complete?'#167846':'#c8102e'}">${r.monthPct}%</b></td><td style="${cell}"><b style="color:${r.behind>0?'#c8102e':'#167846'}">${r.behind>0?this.esc(fmt(r.behind)):'—'}</b>${r.behind>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:105px"><b style="color:${r.complete?'#167846':'#c8102e'}">${this.esc(result)}</b></td></tr>`;
      }).join('')).join('');
      return `<div style="overflow-x:auto"><table style="width:100%;min-width:760px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Level · Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Target Only</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Actual / Early Done</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Cumulative Actual / Site Progress</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Monthly Achieved</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${body}</tbody></table></div>`;
    };
