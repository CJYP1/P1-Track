...
class Component extends DCLogic {
  ...
  _liveReportRows(cat,levels){ const AM=this.ACT_MONTHS,by={},wanted=(levels&&levels.length)?levels:this.DATA.order;
    wanted.forEach(lv=>{ const L=this.DATA.levels[lv]; if(!L)return; (L.zones||[]).forEach(z=>{if((z.cat||'NB')!==cat)return;const zmk=z.mk||z.lid;
      (this._actList(lv,z)||[]).filter(a=>a.custom||this._actApplies(a.id,lv,z)).forEach(a=>{let any=false;for(let i=0;i<AM.length;i++){if(this.actPlan(lv,zmk,a.id,AM[i])!=null||this.actDoneMonth(lv,zmk,a.id,AM[i])!=null){any=true;break;}}if(!any&&!this._actUsesElems(a.id,lv,zmk))return;
        ...
        const k=lv+'\u0001'+a.id;by[k]=by[k]||{a:(filter==='cis'?'Slab CIS':a.label),levels:[lv],aid:a.id,unit:a.unit||'',filter};}); }); });
