...
class Component extends DCLogic {
  ...
  render(){
    ...
    _lbls.forEach(lb=>{
      ...
      if(this.colorMode==='plan'&&this.zoneDelayed(this.curLevel,z,this.planMonth())){const _r=Math.max(fs*0.7,220),_dx=cx,_dy=cy+fs*0.65+_r*1.35;s+=`<circle class="planprog" cx="${_dx.toFixed(0)}" cy="${_dy.toFixed(0)}" r="${(_r*1.15).toFixed(0)}" fill="#e11d2a" fill-opacity="0.25"/><circle class="planprog" cx="${_dx.toFixed(0)}" cy="${_dy.toFixed(0)}" r="${_r.toFixed(0)}" fill="#e11d2a" stroke="#fff" stroke-width="${(_r*0.3).toFixed(0)}"/><circle class="planprog" cx="${_dx.toFixed(0)}" cy="${_dy.toFixed(0)}" r="${(_r*0.34).toFixed(0)}" fill="#fff"/>`;}   /* delay marker(红): 到本月累计计划>累计完成 = 延误 */
      const sub=fs*0.62;
  ...
  _rpRollup(){const PROG=[["Apr","'26"],["May","'26"],["Jun","'26"],["Jul","'26"],["Aug","'26"],["Sep","'26"],["Oct","'26"],["Nov","'26"],["Dec","'26"],["Jan","'27"],["Feb","'27"],["Mar","'27"],["Apr","'27"],["May","'27"]];
    ...
    const _byArea=AREA.map(A=>{const r=agg((z)=>(critMode?z.crit:true)&&(z.cat||'NB')===A.cat);const dl=delayOf(r);return {key:A.key,label:A.label,col:A.col,planned:r.planned,done:r.done,pct:r.planned?Math.round(r.done/r.planned*100):0,doneMonth:r.doneMonth,cum:r.cum,finishIdx:A.finishIdx,finish:A.finish,delayDays:dl.delayDays,behindQty:dl.behindQty,planToDate:dl.planToDate,doneToDate:dl.doneToDate};});
  ...
  _showRPL(){this._rpView=true;this._rpActive='__RPL';const c=this.root.querySelector('#center');if(c)c.style.position='relative';this.root.querySelectorAll('.rbtn').forEach(x=>x.classList.toggle('on',x.dataset.lv==='__RPL'));const hf=this.root.querySelector('#rpframe');if(hf)hf.style.display='none';let f=this.root.querySelector('#rplframe');if(!f){f=document.createElement('iframe');f.id='rplframe';f.setAttribute('sandbox','allow-scripts allow-popups');f.style.cssText='position:absolute;inset:0;width:100%;height:100%;border:0;background:#eef1f5;z-index:60';c.appendChild(f);}const rp=this._rpRollup();const b=this.root.querySelector('#rpLinkedTpl');let tpl=b?this._b64ToUtf8(b.textContent.trim()):'';const inj='<scr'+'ipt>window.__ROLLUP='+JSON.stringify(rp.R)+';window.__ASOF='+JSON.stringify(rp.asOf)+';</scr'+'ipt>';
    const banner='<scr'+'ipt>(function(){var R=window.__ROLLUP||{},A=window.__ASOF||"";function nf(n){return (Math.round(n||0)).toLocaleString();}function mk(){var cd=R.critDelayDays||0,col=cd>0?"#c0392b":"#0f7a52";var pt=R.critPlanToDate||0,dt=R.critDoneToDate||0,bh=R.critBehindQty||0;var areas=(R.byArea||[]).map(function(a){return a.label+" "+(a.delayDays>0?("<b style=\\"color:#c0392b\\">"+a.delayDays+"d</b>"):"<b style=\\"color:#0f7a52\\">on track</b>");}).join(" &nbsp;·&nbsp; ");var host=document.getElementById("app")||document.body;var el=document.createElement("div");el.style.cssText="background:#fff;border:1px solid #e6e9ef;border-left:5px solid "+col+";border-radius:14px;padding:14px 20px;margin:0 0 18px;box-shadow:0 6px 22px rgba(20,30,50,.06);font-family:Poppins,-apple-system,sans-serif";el.innerHTML="<div style=\\"font-size:11px;font-weight:800;color:#98a0ad;letter-spacing:.4px;text-transform:uppercase;margin-bottom:6px\\">Actual vs plan &rarr; schedule delay (as of "+A+" · critical path)</div><div style=\\"display:flex;flex-wrap:wrap;align-items:flex-end;gap:22px\\"><div style=\\"padding-right:20px;border-right:1px solid #eef1f5\\"><div style=\\"font-size:38px;font-weight:800;color:"+col+";line-height:.9\\">"+(cd>0?("~"+cd):"0")+"<span style=\\"font-size:15px\\"> d</span></div><div style=\\"font-size:11.5px;font-weight:700;color:"+col+";margin-top:3px\\">"+(cd>0?"behind schedule":"on track")+"</div></div><div><div style=\\"font-size:20px;font-weight:800\\">"+nf(pt)+"</div><div style=\\"font-size:10.5px;color:#5b6271;font-weight:600\\">should be done by now</div></div><div><div style=\\"font-size:20px;font-weight:800;color:#0f7a52\\">"+nf(dt)+"</div><div style=\\"font-size:10.5px;color:#5b6271;font-weight:600\\">actually done</div></div><div><div style=\\"font-size:20px;font-weight:800;color:"+col+"\\">"+nf(bh)+"</div><div style=\\"font-size:10.5px;color:#5b6271;font-weight:600\\">shortfall</div></div></div><div style=\\"font-size:11px;color:#5b6271;margin-top:9px\\">By area: "+areas+"</div>";host.insertBefore(el,host.firstChild);}if(document.getElementById("app"))mk();else document.addEventListener("DOMContentLoaded",mk);})();</scr'+'ipt>';
    tpl=tpl.replace('</body>',banner+'</body>');tpl=tpl.replace('<body>','<body>'+inj);f.srcdoc=tpl;f.style.display='block';
