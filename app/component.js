...
class Component extends DCLogic {
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0),carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null,behind=carry&&carry.balance>0?carry.balance:0;
      ...
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||0),pourPct=(a.id==='ls'||a.id==='act_corewall')?this.actPourPct(lv,zmk,a.id):null,pct=pourPct!=null?pourPct:(total>0?Math.min(100,Math.round(cumDone/total*100)):(cumPlan>0?Math.min(100,Math.round(cumDone/cumPlan*100)):0)),complete=pourPct!=null?pourPct>=100:(total>0?cumDone>=total:(cumPlan>0&&cumDone>=cumPlan));
      if(!complete&&p<=0&&d<=0&&behind<=0)return;
      rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,done:d,cumPlan,cumDone,total,unit:a.unit||'',pct,complete,critical:!!z.crit,behind,status:complete?'complete':(behind>0?'behind':(d>0?'actual':'track'))});
    });});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0));return `<div style="overflow-x:auto"><table style="width:100%;min-width:590px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Month Plan</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Month Actual</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Cumulative</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Progress</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Status</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.plan,r.unit,false)}</td><td style="padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right;background:${r.done>0?'#edf8f1':'transparent'}">${qty(r.done,r.unit,r.done>0)}</td><td style="padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.cumDone,r.unit,false)}</td><td style="padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right"><b style="color:${r.complete?'#167846':(r.behind>0?'#b54708':'#344054')}">${r.pct}%</b><small style="display:block;margin-top:1px;color:#98a2b3;font-size:7px">${this.esc(fmt(r.cumDone))}/${this.esc(fmt(r.total||r.cumPlan))}</small></td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap">${r.complete?'<b style="color:#167846">Complete</b>':(r.behind>0?`<b style="color:#c8102e">Behind ${this.esc(fmt(r.behind))}${r.unit?' '+this.esc(r.unit):''}</b>`:(r.done>0?'<b style="color:#b54708">In progress</b>':'<b style="color:#667085">Not started</b>'))}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
  ...
  render(){
    ...
    this.svg.querySelectorAll('.liftwall,.liftlbl[data-lwi]').forEach(el=>el.addEventListener('click',ev=>{const idx=+el.dataset.lwi;const slv=el.dataset.lwlv||this.curLevel;ev.stopPropagation();if(this._drawingLift){this._shapeMenu('lift',idx,slv);return;}this._openShape(this._shapeArr('lift',slv)[idx],slv,'stair');}));
