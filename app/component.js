...
class Component extends DCLogic {
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0),carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null,carryBehind=carry&&carry.balance>0?carry.balance:0;
      ...
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||0),scope=total>0?total:cumPlan,pourPct=(a.id==='ls'||a.id==='act_corewall')?this.actPourPct(lv,zmk,a.id):null,monthPct=p>0?Math.min(100,Math.round(d/p*100)):null,monthGap=Math.max(0,p-d),behind=Math.max(carryBehind,monthGap),pct=pourPct!=null?pourPct:(scope>0?Math.min(100,Math.round(cumDone/scope*100)):0),scopeComplete=pourPct!=null?pourPct>=100:(scope>0&&cumDone>=scope),complete=scopeComplete&&behind<=0,overallOver=scope>0?Math.max(0,cumDone-scope):0;
      if(!complete&&p<=0&&d<=0&&behind<=0)return;
      rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,done:d,monthPct,monthGap,cumPlan,cumDone,total,scope,overallOver,unit:a.unit||'',pct,scopeComplete,complete,critical:!!z.crit,behind,carryBehind,status:complete?'complete':(behind>0?'behind':(d>0?'actual':'track'))});
    });});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0)),cell='padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right';return `<div style="overflow-x:auto"><table style="width:100%;min-width:610px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">${this.esc(mon)} Plan / Actual</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">This Month</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Overall Done / Scope</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="5" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}"><b>${this.esc(fmt(r.plan))}</b> / <b style="color:${r.done>0?'#167846':'#667085'}">${this.esc(fmt(r.done))}</b>${r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="${cell}"><b style="font-size:11px;color:${r.monthGap>0?'#c8102e':(r.monthPct!=null?'#167846':'#667085')}">${r.monthPct==null?(r.done>0?'No plan':'—'):r.monthPct+'%'}</b>${r.plan>0?`<small style="display:block;color:#98a2b3;font-size:7px">Actual ${this.esc(fmt(r.done))} ÷ Plan ${this.esc(fmt(r.plan))}</small>`:''}</td><td style="${cell}"><b style="font-size:11px;color:${r.scopeComplete?'#167846':'#b54708'}">${r.pct}%</b><small style="display:block;color:#667085;font-size:7px">${this.esc(fmt(r.cumDone))} done / ${this.esc(fmt(r.scope))} scope</small>${r.overallOver>0?`<small style="display:block;color:#b54708;font-size:7px">Recorded actual is ${this.esc(fmt(r.overallOver))} over scope</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:118px">${r.complete?'<b style="color:#167846">Complete</b><small style="display:block;color:#667085;font-size:7px">Overall done; no monthly shortfall</small>':(r.behind>0?`<b style="color:#c8102e">${this.esc(mon)} outstanding ${this.esc(fmt(r.behind))}${r.unit?' '+this.esc(r.unit):''}</b>`:(r.done>0?'<b style="color:#b54708">In progress</b>':'<b style="color:#667085">Not started</b>'))}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
