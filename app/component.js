...
class Component extends DCLogic {
  ...
  _combinedScheduleLevelRows(cat,lv,mon){
    ...
    this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{
      ...
      const total=Number(this.actTotal(lv,zmk,a.id,a.total)||this.actAutoTotal(lv,zmk,a.id)||0),pourPct=(a.id==='ls'||a.id==='act_corewall')?this.actPourPct(lv,zmk,a.id):null,genuineComplete=pourPct!=null?pourPct>=100:(total>0&&cumDone>=total);
      if(p<=0){if(!genuineComplete)return;rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:0,currentDone:d,currentApplied:0,done:cumDone,applied:cumDone,earlierCredit:0,availableEarlierCredit:0,required:0,previousBacklog:0,monthPct:100,monthGap:0,cumPlan,cumDone,total,unit:a.unit||'',complete:true,completedEarlier:true,genuineComplete:true,critical:!!z.crit,behind:0,status:'complete'});return;}
      const previousPlan=Math.max(0,cumPlan-p),previousDone=Math.max(0,cumDone-d),availableEarlierCredit=Math.max(0,previousDone-previousPlan),required=p+previousBacklog,currentApplied=Math.min(required,d),earlierCredit=Math.min(availableEarlierCredit,Math.max(0,required-currentApplied));let applied=currentApplied+earlierCredit,behind=Math.max(0,required-applied),complete=behind<=0;if(genuineComplete){applied=required;behind=0;complete=true;}const monthPct=required>0?Math.min(100,Math.round(applied/required*100)):null;
      rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,currentDone:d,currentApplied,done:applied,applied,earlierCredit,availableEarlierCredit,required,previousBacklog,monthPct,monthGap:behind,cumPlan,cumDone,total,unit:a.unit||'',complete,completedEarlier:false,genuineComplete,critical:!!z.crit,behind,status:complete?'complete':(applied>0?'actual':'behind')});
    });});
  ...
  _exportCombinedMonthlyPng(cat,lv,mon){
    ...
    x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle=meta.color;x.fillRect(0,0,W,80);x.fillStyle='#fff';x.font='700 27px Arial';x.fillText('P1 Waterfront · '+meta.name+' · '+lv,34,35);x.font='700 16px Arial';x.fillText(mon+' · Required Target versus Progress Applied',34,62);const cols=[34,430,680,875,1035,1200];let y=108;x.font='700 13px Arial';x.fillStyle='#475467';['Zone','Required Target','Progress Applied','Achieved','Outstanding','Result'].forEach((h,i)=>x.fillText(h,cols[i],y));y+=20;x.strokeStyle='#cfd7e2';x.beginPath();x.moveTo(28,y);x.lineTo(W-28,y);x.stroke();
    acts.forEach(aid=>{const g=by[aid];if(y+headH>H)return;x.fillStyle='#e8edf3';x.fillRect(28,y,W-56,headH);x.fillStyle='#202938';x.font='700 16px Arial';x.fillText(g.name,40,y+25);y+=headH;g.rows.sort((a,b)=>(b.complete-a.complete)||String(a.zone).localeCompare(String(b.zone))).forEach(r=>{if(y+rowH>H)return;x.fillStyle=r.complete?'#edf8f1':'#fff';x.fillRect(28,y,W-56,rowH);x.fillStyle='#202938';x.font='700 13px Arial';x.fillText(r.zone+(r.critical?'  CRITICAL':''),cols[0],y+25);x.font='13px Arial';x.fillStyle=r.completedEarlier?'#167846':'#202938';x.fillText(r.completedEarlier?'Completed earlier':fmt(r.required)+(r.unit?' '+r.unit:'')+'  (Plan '+fmt(r.plan)+(r.previousBacklog>0?' + backlog '+fmt(r.previousBacklog):'')+')',cols[1],y+25);x.fillStyle=r.applied>0?'#167846':'#202938';x.fillText(r.completedEarlier?fmt(r.cumDone)+(r.unit?' '+r.unit:'')+' cumulative complete':fmt(r.applied)+(r.unit?' '+r.unit:'')+'  ('+mon+' '+fmt(r.currentDone)+(r.earlierCredit>0?' + earlier '+fmt(r.earlierCredit):'')+')',cols[2],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.monthPct+'%',cols[3],y+25);x.fillStyle=r.behind>0?'#c8102e':'#667085';x.fillText(r.behind>0?fmt(r.behind)+(r.unit?' '+r.unit:''):'—',cols[4],y+25);x.fillStyle=r.complete?'#167846':'#c8102e';x.fillText(r.completedEarlier?'Completed earlier':(r.complete?'Plan + previous backlog achieved':mon+' outstanding'),cols[5],y+25);x.strokeStyle='#e4e8ee';x.beginPath();x.moveTo(28,y+rowH);x.lineTo(W-28,y+rowH);x.stroke();y+=rowH;});y+=8;});
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0)),cell='padding:7px 6px;border-bottom:1px solid #e7ebf0;text-align:right';return `<div style="overflow-x:auto"><table style="width:100%;min-width:610px;border-collapse:collapse;background:#fff;color:#202938;font-size:9px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Required Target</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Progress Applied</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Achieved</th><th style="padding:7px 6px;border-bottom:1px solid #d9e0e8;text-align:right">Outstanding</th><th style="padding:7px 8px;border-bottom:1px solid #d9e0e8">Result</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:5px 8px;background:#e5eaf1;color:#344054;font-size:9px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="${cell}">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':qty(r.required,r.unit,false)+`<small style="display:block;color:#667085;font-size:7px">Plan ${this.esc(fmt(r.plan))}${r.previousBacklog>0?' + previous backlog '+this.esc(fmt(r.previousBacklog)):''}</small>`}</td><td style="${cell};background:${r.applied>0?'#edf8f1':'transparent'}">${qty(r.applied,r.unit,r.applied>0)}<small style="display:block;color:#667085;font-size:7px">${r.completedEarlier?'Cumulative completed quantity':this.esc(mon)+' Actual '+this.esc(fmt(r.currentDone))+(r.earlierCredit>0?' + earlier completion '+this.esc(fmt(r.earlierCredit)):'')}</small></td><td style="${cell}"><b style="font-size:11px;color:${r.complete?'#167846':'#c8102e'}">${r.monthPct}%</b></td><td style="${cell}"><b style="color:${r.behind>0?'#c8102e':'#167846'}">${r.behind>0?this.esc(fmt(r.behind)):'—'}</b>${r.behind>0&&r.unit?`<small style="display:block;color:#667085;font-size:8px">${this.esc(r.unit)}</small>`:''}</td><td style="padding:7px 8px;border-bottom:1px solid #e7ebf0;white-space:normal;min-width:105px">${r.completedEarlier?'<b style="color:#167846">Completed earlier</b>':(r.complete?'<b style="color:#167846">Plan + previous backlog achieved</b>':`<b style="color:#c8102e">${this.esc(mon)} outstanding</b>`)}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
    ...
    [...ov.querySelectorAll('div,span')].forEach(el=>{const t=el.textContent.trim();if(!el.querySelector('div,span')&&t.startsWith('✓ Complete · overall scope done'))el.textContent='✓ Complete / Required Target achieved';else if(!el.querySelector('div,span')&&t==='No work meets both completion conditions yet.')el.textContent='No completed or achieved work yet.';else if(!el.querySelector('div,span')&&t.startsWith('● In progress / monthly outstanding'))el.textContent=t.replace('● In progress / monthly outstanding','● Required Target outstanding');else if(!el.querySelector('div,span')&&t.startsWith('How this report decides “Complete”'))el.innerHTML=`<b>Included rows:</b> Zones with a ${this.esc(mon)} Plan, plus activities genuinely completed earlier. <b>Required Target</b> = ${this.esc(mon)} Plan + previous backlog. <b>Progress Applied</b> = ${this.esc(mon)} Actual + eligible earlier completion. A fully completed activity stays under <b style="color:#167846">Complete</b> even when its ${this.esc(mon)} Plan is zero.`;else if(el.tagName==='SPAN'&&!el.querySelector('*')&&t.includes(' zones · '+mon+' Plan ')&&t.includes(' · Actual '))el.textContent=t.replace(' · Actual ',' · Applied ');});
