...
class Component extends DCLogic {
  ...
  _zoneWorkerKey(lv,zmk,date,field){return lv+'||'+zmk+'||zone_daily||'+date+'||'+field;}
  _zoneWorkerVal(lv,zmk,date,field){const v=(this._manpower||{})[this._zoneWorkerKey(lv,zmk,date,field)];return v==null?null:Math.max(0,Math.round(Number(v)||0));}
  setZoneWorkerVal(lv,zmk,date,field,raw){if(!(this.rwsIsAdmin()||this.rwsScopeOk(lv,zmk))){this.rwsDeny('Outside your assigned zones.');return;}if(!/^\d{4}-\d{2}-\d{2}$/.test(date||'')||!['cjy_day','cjy_night','sub_day','sub_night'].includes(field))return;const k=this._zoneWorkerKey(lv,zmk,date,field),s=String(raw==null?'':raw).trim(),v=s===''?null:Math.max(0,Math.round(Number(s)||0));this._manpower=this._manpower||{};if(v==null)delete this._manpower[k];else this._manpower[k]=v;try{localStorage.setItem('rws_manpower',JSON.stringify(this._manpower));}catch(e){}if(typeof rwsSyncKV==='function')rwsSyncKV('manpower',k,v,lv,zmk);}
  _zoneWorkerZones(){const out=[],seen=new Set(),add=(lv,z)=>{if(!z)return;const zmk=z.mk||z.lid||'',k=lv+'||'+zmk;if(!zmk||seen.has(k))return;seen.add(k);out.push({lv,zmk,label:z.label||zmk.split('|').pop(),cat:z.cat||this.zoneCat(lv,zmk)||'NB'});};(this.DATA.order||[]).forEach(lv=>((this.DATA.levels[lv]&&this.DATA.levels[lv].zones)||[]).forEach(z=>add(lv,z)));const S=this.SUBZONES&&this.SUBZONES.L1;['C','P'].forEach(kind=>(S&&S[kind]||[]).forEach(e=>add('L1',{mk:'L1|'+e.label,label:e.label,cat:'MA'})));return out.sort((a,b)=>['NB','EB','MA'].indexOf(a.cat)-['NB','EB','MA'].indexOf(b.cat)||(this.DATA.order.indexOf(a.lv)-this.DATA.order.indexOf(b.lv))||String(a.label).localeCompare(String(b.label)));}
  openManpower(){ const {byDay, byZoneDay, zones}=this._allDailyManpower();
    ...
    const zoneDate=/^\d{4}-\d{2}-\d{2}$/.test(this._zoneWorkerDate||'')?this._zoneWorkerDate:this.todayISOStr(),zoneArea=['ALL','NB','EB','MA'].includes(this._zoneWorkerArea)?this._zoneWorkerArea:'ALL',zoneFields=['cjy_day','cjy_night','sub_day','sub_night'],zoneRows=this._zoneWorkerZones().filter(z=>zoneArea==='ALL'||z.cat===zoneArea);this._zoneWorkerDate=zoneDate;this._zoneWorkerArea=zoneArea;
    let zoneGrand=0,lastGroup='';const zoneBody=zoneRows.map(z=>{const vals=zoneFields.map(f=>this._zoneWorkerVal(z.lv,z.zmk,zoneDate,f)||0),total=vals.reduce((n,v)=>n+v,0),can=this.rwsIsAdmin()||this.rwsScopeOk(z.lv,z.zmk),group=z.cat+' · '+z.lv,groupRow=group!==lastGroup?(lastGroup=group,`<tr><td colspan="7" style="padding:7px 10px;background:#e8edf3;color:#344054;font-size:10px;font-weight:900">${this.esc(group)}</td></tr>`):'';zoneGrand+=total;const cells=zoneFields.map((f,i)=>`<td style="padding:6px 8px;border-bottom:1px solid var(--line);text-align:center">${can?`<input class="zone-worker-in" data-lv="${this.esc(z.lv)}" data-zmk="${this.esc(z.zmk)}" data-field="${f}" type="number" min="0" step="1" value="${vals[i]||''}" placeholder="—" style="width:62px;padding:5px;border:1px dashed var(--accent);border-radius:6px;background:var(--panel);color:var(--txt);text-align:center;font-weight:700">`:(vals[i]||'—')}</td>`).join('');return groupRow+`<tr class="zone-worker-row"><td style="padding:7px 10px;border-bottom:1px solid var(--line);font-weight:800"><span style="color:var(--faint);font-size:9px;margin-right:6px">${this.esc(z.lv)}</span>${this.esc(z.label)}</td><td style="padding:7px 8px;border-bottom:1px solid var(--line);font-weight:800;color:${z.cat==='NB'?'#e58b35':z.cat==='EB'?'#d95d92':'#3478c9'}">${this.esc(z.cat==='MA'?'MR':z.cat)}</td>${cells}<td class="zone-worker-total" style="padding:7px 10px;border-bottom:1px solid var(--line);text-align:right;font-size:13px;font-weight:900">${total}</td></tr>`;}).join('')||'<tr><td colspan="7" style="padding:28px;text-align:center;color:var(--faint)">No zones found.</td></tr>';
    const zoneTable=`<section style="border:1px solid var(--line);border-radius:11px;background:var(--panel);overflow:hidden"><div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;padding:10px 13px;background:var(--panel2);border-bottom:1px solid var(--line)"><b style="font-size:14px">Daily workers by zone</b><input id="zoneWorkerDate" type="date" value="${zoneDate}" style="padding:5px 8px;border:1px solid var(--line);border-radius:7px;background:var(--panel);color:var(--txt)"><select id="zoneWorkerArea" style="padding:5px 8px;border:1px solid var(--line);border-radius:7px;background:var(--panel);color:var(--txt)">${[['ALL','All areas'],['NB','NB'],['EB','EB'],['MA','MR']].map(x=>`<option value="${x[0]}"${zoneArea===x[0]?' selected':''}>${x[1]}</option>`).join('')}</select><span style="margin-left:auto;font-size:11px;color:var(--dim)">Daily total <b id="zoneWorkerGrand" style="font-size:17px;color:var(--txt)">${zoneGrand}</b></span></div><div style="overflow:auto;max-height:72vh"><table style="width:100%;min-width:850px;border-collapse:collapse;font-size:11px"><thead style="position:sticky;top:0;background:#202b52;color:#fff;z-index:2"><tr><th rowspan="2" style="padding:8px 10px;text-align:left">Zone</th><th rowspan="2" style="padding:8px">Area</th><th colspan="2" style="padding:7px;border-left:1px solid #fff4">CJY RC team</th><th colspan="2" style="padding:7px;border-left:1px solid #fff4">New RC sub-con</th><th rowspan="2" style="padding:8px;text-align:right">Total</th></tr><tr><th style="padding:6px">Day</th><th style="padding:6px">Night</th><th style="padding:6px">Day</th><th style="padding:6px">Night</th></tr></thead><tbody>${zoneBody}</tbody></table></div><div style="padding:8px 12px;background:var(--panel2);font-size:9.5px;color:var(--dim)">This table is independent from Activity manpower. Everyone can view it; assigned area users can update their own zones, and admin can update all zones.</div></section>`;
    ...
    ov.innerHTML=`<div style="max-width:1450px;margin:0 auto">
      ...
      <div class="seg" id="mpwView" style="margin:0 0 12px"><button data-view="zone" class="${view==='zone'?'on':''}">Daily workers by zone</button>${admin?`<button data-view="daily" class="${view==='daily'?'on':''}">Activity daily summary</button><button data-view="summary" class="${view==='summary'?'on':''}">Summary by zone</button>`:''}</div>
      ${view==='zone'?zoneTable:`<div style="font-size:12px;color:var(--dim);margin-bottom:12px">All daily entries from every zone and activity · C = Carpenter · R = Ironworker · Project peak <b>${peak}</b> workers</div>${view==='daily'?dailyTable:`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:14px;align-items:start">${groupTables}</div>`}`}</div>`;
    ov.style.display='block'; const cl=ov.querySelector('#mpwClose'); if(cl)cl.onclick=()=>{ov.style.display='none';};ov.querySelectorAll('#mpwView button').forEach(b=>b.onclick=()=>{this._manpowerView=b.dataset.view;this.openManpower();});const zd=ov.querySelector('#zoneWorkerDate');if(zd)zd.onchange=e=>{this._zoneWorkerDate=e.target.value;this.openManpower();};const za=ov.querySelector('#zoneWorkerArea');if(za)za.onchange=e=>{this._zoneWorkerArea=e.target.value;this.openManpower();};ov.querySelectorAll('.zone-worker-in').forEach(inp=>inp.onchange=()=>{this.setZoneWorkerVal(inp.dataset.lv,inp.dataset.zmk,zoneDate,inp.dataset.field,inp.value);const tr=inp.closest('tr'),sum=[...tr.querySelectorAll('.zone-worker-in')].reduce((n,x)=>n+(Number(x.value)||0),0);tr.querySelector('.zone-worker-total').textContent=sum;const grand=[...ov.querySelectorAll('.zone-worker-total')].reduce((n,x)=>n+(Number(x.textContent)||0),0),g=ov.querySelector('#zoneWorkerGrand');if(g)g.textContent=grand;}); }
  ...
  rwsRenderUserBar(){
    ...
    const adminOnly=['#saveLock','#exportXls','#openTable','#exportJson','#rwsChangesBtn','#openMonthlySummary','#openDelayAdmin'].map(s=>this.root.querySelector(s)).filter(Boolean);   /* Manpower 里的 Daily workers by zone 给所有登录用户查看；汇总 tabs 仍只给 admin */
