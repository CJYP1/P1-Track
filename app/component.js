...
class Component extends DCLogic {
  ...
  _elemDisplayId(lv,type,id){const raw=String(id==null?'':id);if(type!=='beam')return raw;const m=String(lv||'').match(/^L([34])$/i);if(!m)return raw;return raw.replace(/^2HB/i,m[1]+'H').replace(/^2VB/i,m[1]+'V');}
  ...
  rwsRenderUserBar(){
    ...
    if(!u){info.textContent='';lo.style.display='none';ab.style.display='none';adminOnly.forEach(b=>b.style.display='none');if(manpowerBtn)manpowerBtn.style.display='none';if(sched)sched.style.display='none';return;}
  ...
  async rwsRenderAdmin(){
    ...
    const AL={EB:'Existing Basement',NB:'New Basement',MA:'Marine',RWS:'RWS · all Reports (view only)',REPEDIT:'Report Edit',PLAN:'Planning Comments',CMT:'Comment (main + M28)',M28_OTHER:'M28 · L2/L3/L4/Ramp/TST',HIST:'History (snapshots)'};
    ...
    try{
      ...
      const AREAS=[['EB','Existing Basement'],['NB','New Basement'],['MA','Marine'],['RWS','RWS · all Reports (view only)'],['REPEDIT','Report Edit (combine with NB / EB / Marine)'],['PLAN','Planning Comments (To Planning)'],['CMT','Comment (main map + M28)'],['M28_OTHER','M28 · L2/L3/L4 · Ramp · TST · Hoarding'],['HIST','🕓 History / 历史快照 (view snapshots)']];
      body.innerHTML=`<div style="margin-bottom:14px;border:1px solid var(--line);border-radius:10px;padding:12px;background:var(--panel2)">
        ...
        <div style="font-size:9.5px;color:var(--faint);margin-top:6px">A "user" account can change status and actual completed quantities only inside the areas checked above. Report editing needs <b>Report Edit</b> plus the matching NB / EB / Marine area; RWS remains view-only. The M28 board reuses the area permissions. Main-map commenting needs the Comment box; Report comments are available to the matching Report editors/area team. Admins can edit everything and manage accounts.</div>
      </div>
  ...
  _rpAugPct(lv,z){if(!z)return null;const n=s=>String(s||'').toUpperCase().replace(/\s+/g,'').replace(/^POD/,''),lab=n(z.label),cat=z.cat||'NB';
    ...
    if(cat==='MA'){
      ...
      if(lv==='L3'||lv==='L4')return /^(?:1\.[12](?:T)?|3\.[1-4](?:CIST|T)?)$/.test(lab)?0:null;
      return null;
    }
  ...
  _reportDefs(){ return {
    NB:{label:'NB Report', title:'New Basement Superstructure', scope:'New Basement · working levels with live Activity data',levels:['B2','B1','B1M','L1','L2','L3','L4']},
    EB:{label:'EB Report', title:'Existing Basement Superstructure', scope:'Existing Basement · working levels with live Activity data',levels:['B2','B1','B1M','L1','L2','L3','L4']},
    MA:{label:'MA Report', title:'Marine Superstructure', scope:'Marine · working levels with live Activity data',levels:['B2','B1','B1M','L1','L2','L3','L4']} }; }
