...
class Component extends DCLogic {
  ...
  _renderResourcePanel(){if(!this._resourceMode)return;const side=this.root.querySelector('#side'),title=this.root.querySelector('#sideTitle'),kpis=this.root.querySelector('#kpis'),prog=this.root.querySelector('#progroll'),foot=this.root.querySelector('#foot'),p=this.root.querySelector('#sidebody');if(!side||!p)return;const old=this.root.querySelector('#resourcePlannerPanel');if(old)old.remove();p.id='sidebody';p.dataset.resource='1';if(title)title.textContent='🧰 Resource Planning';if(kpis)kpis.style.display='none';if(prog)prog.style.display='none';if(foot)foot.style.display='none';const d=this._resourceData(),admin=this.rwsIsAdmin(),editing=admin&&!!this._resourceEditing,t=this._resourceTeam(),colors=this._resourceColours(),n=v=>Number(v)||0,zs=t?(t.zones||[]):[],tot=zs.reduce((a,x)=>{a.f+=n(x.formwork);a.c+=n(x.concrete);a.t+=n(x.total);a.w+=n(x.workers);return a;},{f:0,c:0,t:0,w:0}),selected=this._resourceInspect&&this._resourceEntry(this._resourceInspect.lv,this._resourceInspect.zmk);
    ...
    const cards=zs.map((x,i)=>{const on=this._resourceInspect&&this._resourceInspect.lv===x.lv&&this._resourceInspect.zmk===x.zmk;return `<section class="resource-zone-card" data-i="${i}" style="margin-bottom:8px;padding:9px;border:${on?'2px':'1px'} solid ${on?t.color:'var(--line)'};border-radius:9px;background:${on?'color-mix(in srgb,'+t.color+' 8%,var(--panel))':'var(--panel)'}"><div style="display:flex;align-items:center;gap:7px"><b style="display:grid;place-items:center;width:25px;height:25px;border-radius:50%;background:${t.color};color:#fff">${i+1}</b><div style="min-width:0;flex:1"><b>${this.esc(x.lv)} · ${this.esc(this._resourceZoneLabel(x.lv,x.zmk))}</b><small style="display:block;color:${t.color};font-weight:800">${this.esc(t.name)} · Work order ${i+1}</small></div>${editing?`<button class="resource-up" title="Move earlier" ${i===0?'disabled':''}>↑</button><button class="resource-down" title="Move later" ${i===zs.length-1?'disabled':''}>↓</button><button class="resource-remove" title="Remove" style="color:var(--crit)">✕</button>`:''}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:9px">${input(x,'workers','Workers 人数')}${input(x,'formwork','Formwork 模板量')}${input(x,'concrete','Concrete 洋灰量')}${input(x,'total','Total 总量')}</div></section>`;}).join('')||`<div style="padding:24px 12px;text-align:center;color:var(--faint);border:1px dashed var(--line);border-radius:9px">${editing?'Select a Team, then click Zones on the map in work order.':'No Zones assigned to this team.'}</div>`;
    ...
    p.innerHTML=`<div style="padding:10px 10px 18px"><div style="display:flex;align-items:center;gap:6px"><div style="font-size:10px;color:var(--dim);line-height:1.4;flex:1">${editing?'Edit mode: choose a Team, click Zones in work order, then enter resources.':'View mode: click a coloured Zone to inspect its Team, order and resources.'}</div>${admin?`<button class="hbtn ${editing?'primary':''}" id="resourceEdit" style="padding:5px 9px">${editing?'Done':'Edit'}</button>`:''}<button class="hbtn" id="resourceClose" style="padding:5px 8px">Exit</button></div><div style="display:flex;gap:5px;flex-wrap:wrap;margin:10px 0">${tabs}${editing?'<button class="hbtn" id="resourceAddTeam" style="padding:5px 8px">+ Team</button>':''}</div>${selectedNote}${t?`<div style="display:flex;align-items:center;gap:7px;margin:9px 0;padding:8px;background:var(--panel2);border-radius:8px">${editing?`<input id="resourceTeamColor" type="color" value="${t.color}" title="Team colour" style="width:30px;height:30px;padding:1px;border:1px solid var(--line);border-radius:6px"><input id="resourceTeamName" value="${this.esc(t.name)}" style="min-width:0;flex:1;font-weight:900;padding:6px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt)">`:`<span style="width:12px;height:12px;border-radius:50%;background:${t.color}"></span><b style="flex:1">${this.esc(t.name)}</b>`}<span style="font-size:9px;color:var(--dim)">${zs.length} Zones</span>${editing&&d.teams.length>1?'<button id="resourceDeleteTeam" style="border:0;background:transparent;color:var(--crit);cursor:pointer">Delete</button>':''}</div>${cards}<div style="padding:9px;border-top:3px solid ${t.color};background:var(--panel2);border-radius:8px;font-size:10px"><b>TEAM TOTAL</b><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:5px"><span>Workers <b>${this.fmt(tot.w)}</b></span><span>Total <b>${this.fmt(tot.t)}</b></span><span>Formwork <b>${this.fmt(tot.f)}</b></span><span>Concrete <b>${this.fmt(tot.c)}</b></span></div></div>`:'<div class="empty">No teams yet.</div>'}</div>`;
  ...
  openLookAhead(){
    ...
    const combinedBtn=ov.querySelector('[data-view="combined"]');if(combinedBtn)combinedBtn.onclick=()=>{this._reportEditing=false;this._reportCombinedArea=cat;this._reportCombined=true;this.openLookAhead();};
  ...
  bindGlobal(){
    ...
    {const _la=this.root.querySelector('#openLookAhead');if(_la)_la.addEventListener('click',()=>{const cats=this._reportCats();this._reportEditing=false;this._reportCombined=true;this._reportCombinedArea=cats.indexOf(this._reportCat)>=0?this._reportCat:(cats[0]||'NB');this.openLookAhead();});}
