...
class Component extends DCLogic {
  ...
  openMonthlySummary(){
    ...
    const makePath=(title,list,critical)=>{let lastLv='';const body=list.map(r=>{const head=r.lv!==lastLv?(lastLv=r.lv,`<tr class="monthly-level"><td colspan="7">${this.esc(cat)} · ${this.esc(r.lv)}</td></tr>`):'';const cell=(v,cls)=>`<td class="num ${cls||''}">${fmt(v)}<small>${this.esc(r.unit)}</small></td>`;return head+`<tr><td><b>${this.esc(r.label)}</b><small>${r.zones.size} zone${r.zones.size===1?'':'s'}</small></td>${cell(r.earlyRemain,r.earlyRemain?'backlog':'')}${cell(r.prevPlan)}${cell(r.plan)}${cell(r.prevDone,'done')}${cell(r.done,'done')}${cell(r.owe,r.owe?'owe':'clear')}</tr>`;}).join('')||`<tr><td colspan="7" class="monthly-empty">No ${critical?'critical':'non-critical'} work for ${this.esc(mon)}.</td></tr>`;return `<section class="monthly-path ${critical?'critical':'noncritical'}"><h3>${title}</h3><div class="monthly-summary-table"><table><thead><tr><th>Remaining Activity</th><th>${this.esc(earlier)}<small>remaining</small></th><th>${this.esc(prev)}<small>plan</small></th><th>${this.esc(mon)}<small>plan</small></th><th>${this.esc(prev)}<small>actual work</small></th><th>${this.esc(mon)}<small>actual work</small></th><th>${this.esc(mon)}<small>owed after month</small></th></tr></thead><tbody>${body}</tbody></table></div></section>`;};
