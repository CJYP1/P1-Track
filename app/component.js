...
class Component extends DCLogic {
  ...
  _reportEditedValues(cat,r,A,P){const key=this._reportEditKey(cat,r),all=(this._appCfg&&this._appCfg.reportOverrides)||{},o=all[key]||{},num=(v,d)=>Number.isFinite(Number(v))?Math.max(0,Math.round(Number(v))):Math.max(0,Math.round(Number(d)||0));const total=num(o.total,A.total||P.total),done=Math.min(total,num(o.done,A.done)),planned=Math.min(total,num(o.planned,P.planned));return {key,has:Object.prototype.hasOwnProperty.call(all,key),A:{...A,done,total,pct:total?Math.min(100,Math.round(done/total*100)):0},P:{...P,planned,total}};}
  ...
  openLookAhead(){
    ...
    const makeRows=(reportRows)=>reportRows.map(r=>{ const liveA=r.actual||this._catchupActual(r.levels,r.aid,cat,r.filter),liveP=r.plan||this._catchupPlan(r.levels,r.aid,cat,r.filter),ev=this._reportEditedValues(cat,r,liveA,liveP),A=ev.A,P=ev.P,den=A.total||P.total,tgt=den>0?Math.min(100,Math.round(P.planned/den*100)):0,unit=r.unit?(' '+r.unit):''; const actSub=`(${fmtN(A.done)}/${fmtN(A.total)}${unit})`,planSub=`(${fmtN(P.planned)}/${fmtN(den)}${unit} planned to date)`,by=P.end?this._fmtDShort(P.end):'—',inp='width:70px;padding:5px 6px;border:1px solid #cbaeb4;border-radius:5px;text-align:center;font:700 12px Segoe UI;background:#fff;color:#2b1114';
      const planEdit=editing?`<div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap;margin-top:6px"><label style="font-size:9px">Catch-Up<br><input class="rpt-edit" data-field="planned" type="number" min="0" step="1" value="${fmtN(P.planned)}" data-live="${fmtN(liveP.planned)}" style="${inp}"></label><label style="font-size:9px">Total<br><input class="rpt-edit" data-field="total" type="number" min="0" step="1" value="${fmtN(den)}" data-live="${fmtN(liveA.total||liveP.total)}" style="${inp}"></label></div>`:'';
    ...
    const makeTable=(lv)=>{const rows=makeRows(this._liveReportRows(cat,[lv]))||`<tr><td colspan="3" style="background:#f4dbdf;color:#6d1327;text-align:center;padding:18px 8px;font-size:11px">当前 HTML 数据源里还没有 ${this.esc(cat+' '+lv)} Structure Activity 数据。</td></tr>`;return `<section style="min-width:0"><div style="font-size:14px;font-weight:900;color:#6d1327;margin:0 0 5px 5px">${this.esc(lv)} Structure</div><div style="background:#efe9df;padding:4px;border-radius:5px;width:100%"><table style="width:100%;border-collapse:separate;border-spacing:4px;font-family:'Segoe UI',Arial,sans-serif"><thead><tr>`
      ...
      +`<th style="background:#6d1327;color:#fff;padding:11px 8px;font-size:11px;font-weight:800;text-align:center;line-height:1.25">CJY’s Catch-Up<br>${this.esc(this._reportDateLabel(this._reportToday()))}</th>`
      +`<th style="background:#6d1327;color:#fff;padding:11px 8px;font-size:11px;font-weight:800;text-align:center">Actual</th>`
