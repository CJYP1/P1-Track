...
class Component extends DCLogic {
  ...
  actPourStats(lv,zmk,aid){const pre=lv+'||'+zmk+'||'+aid+'||pour_elem||',vals=[];Object.keys(this._manpower||{}).forEach(k=>{if(k.indexOf(pre)!==0)return;const v=this._manpower[k];if(v!=null&&Number.isFinite(Number(v)))vals.push(Math.max(0,Math.min(100,Math.round(Number(v)))));});return {count:vals.length,sum:vals.reduce((n,v)=>n+v,0),pct:vals.length?Math.round(vals.reduce((n,v)=>n+v,0)/vals.length):null};}
  ...
  openManpower(){ const {byDay, byZoneDay, zones}=this._allDailyManpower();
    ...
    const view=this._manpowerView==='summary'?'summary':'daily';this._manpowerView=view;
    ov.innerHTML=`<div style="max-width:1450px;margin:0 auto">
      ...
      <div class="seg" id="mpwView" style="margin:0 0 12px"><button data-view="daily" class="${view==='daily'?'on':''}">Daily</button><button data-view="summary" class="${view==='summary'?'on':''}">Summary by zone</button></div>
      <div style="font-size:12px;color:var(--dim);margin-bottom:12px">All daily entries from every zone and activity · C = Carpenter · R = Ironworker · Project peak <b>${peak}</b> workers</div>
      ${view==='daily'?dailyTable:`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:14px;align-items:start">${groupTables}</div>`}</div>`;
    ov.style.display='block'; const cl=ov.querySelector('#mpwClose'); if(cl)cl.onclick=()=>{ov.style.display='none';};ov.querySelectorAll('#mpwView button').forEach(b=>b.onclick=()=>{this._manpowerView=b.dataset.view;this.openManpower();}); }
  ...
  _reportDefs(){ return {
    ...
    EB:{label:'EB Report', title:'Existing Basement Superstructure', scope:'Existing Basement · B2 / B1 / L1 / L2',levels:['B2','B1','L1','L2']},
    MA:{label:'MA Report', title:'Marine Superstructure', scope:'Marine · L1 / L2 / L3 / L4',levels:['L1','L2','L3','L4']} }; }
  _reportEditKey(cat,r){return [cat,(r.levels&&r.levels[0])||'',r.aid||'',r.filter||''].join('||');}
  _reportEditedValues(cat,r,A,P,liveTotal){const key=this._reportEditKey(cat,r),cfg=this._appCfg||{},all={...(cfg.reportOverrides||{}),...(cfg['reportOverrides:'+cat]||{})},o=all[key]||{},num=(v,d)=>Number.isFinite(Number(v))?Math.max(0,Math.round(Number(v))):Math.max(0,Math.round(Number(d)||0));const total=num(o.total,liveTotal),done=Math.min(total,num(o.done,A.done)),planned=Math.min(total,num(o.planned,P.planned));return {key,has:Object.prototype.hasOwnProperty.call(all,key),A:{...A,done,total,pct:total?Math.min(100,Math.round(done/total*100)):0},P:{...P,planned,total}};}
  ...
  _combinedScheduleLevelRows(cat,lv,mon){const mi=(this.ACT_MONTHS||[]).indexOf(mon),rows=[];this._reportZones(lv,cat).forEach(z=>{const zmk=z.mk||z.lid,zone=z.label||zmk;this._schZoneActs(lv,z).forEach(a=>{const p=Number(this.actPlan(lv,zmk,a.id,mon)||0),d=Number(this.actDoneMonth(lv,zmk,a.id,mon)||0),carry=mi>=0?this.actCarry(lv,zmk,a.id,mi):null,behind=carry&&carry.balance>0?carry.balance:0;if(p<=0&&d<=0&&behind<=0)return;let cumPlan=0,cumDone=0;for(let j=0;j<=mi;j++){cumPlan+=Number(this.actPlan(lv,zmk,a.id,this.ACT_MONTHS[j])||0);cumDone+=Number(this.actDoneMonth(lv,zmk,a.id,this.ACT_MONTHS[j])||0);}const pct=cumPlan>0?Math.min(100,Math.round(cumDone/cumPlan*100)):(cumDone>0?100:0);rows.push({cat,lv,zmk,zone,aid:a.id,name:a.label,plan:p,done:d,cumPlan,cumDone,unit:a.unit||'',pct,critical:!!z.crit,behind,status:behind>0?'behind':(d>0?'actual':'track')});});});return rows.sort((a,b)=>String(a.zone).localeCompare(String(b.zone))||String(a.name).localeCompare(String(b.name)));}
  ...
  _renderCombinedReport(ov,cats,close){
    ...
    const tableHtml=rows=>{const levels=(defs[rows[0]&&rows[0].cat]||{}).levels||this.DATA.order||[],byLv={};rows.forEach(r=>(byLv[r.lv]||(byLv[r.lv]=[])).push(r));const order=levels.filter(lv=>byLv[lv]).concat(Object.keys(byLv).filter(lv=>levels.indexOf(lv)<0));return `<div style="overflow-x:auto"><table style="width:100%;min-width:690px;border-collapse:collapse;background:#fff;color:#202938;font-size:10px"><thead><tr style="background:#eef1f5;color:#475467;text-align:left"><th style="padding:8px 10px;border-bottom:1px solid #d9e0e8">Zone</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Month Plan</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Month Actual</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Cumulative Actual</th><th style="padding:8px 9px;border-bottom:1px solid #d9e0e8;text-align:right">Progress</th><th style="padding:8px 10px;border-bottom:1px solid #d9e0e8">Status</th></tr></thead><tbody>${order.map(lv=>`<tr><td colspan="6" style="padding:6px 10px;background:#e5eaf1;color:#344054;font-size:10px;font-weight:900;letter-spacing:.35px">LEVEL ${this.esc(lv)} · ${byLv[lv].length} zone${byLv[lv].length===1?'':'s'}</td></tr>`+byLv[lv].map(r=>`<tr><td style="padding:8px 10px;border-bottom:1px solid #e7ebf0;white-space:nowrap"><button type="button" class="combined-zone-open" data-lv="${this.esc(r.lv)}" data-zmk="${this.esc(r.zmk)}" style="border:0;background:none;padding:0;color:#202938;font:inherit;font-weight:900;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px" title="Open zone details">${this.esc(r.zone)} ↗</button>${r.critical?'<small style="display:block;color:#c8102e;font-size:7.5px;font-weight:900;margin-top:2px">CRITICAL</small>':''}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.plan,r.unit,false)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right;background:${r.done>0?'#edf8f1':'transparent'}">${qty(r.done,r.unit,r.done>0)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right">${qty(r.cumDone,r.unit,false)}</td><td style="padding:8px 9px;border-bottom:1px solid #e7ebf0;text-align:right"><b style="color:${r.behind>0?'#b54708':'#167846'}">${r.pct}%</b><small style="display:block;margin-top:1px;color:#98a2b3;font-size:8px">${this.esc(fmt(r.cumDone))}/${this.esc(fmt(r.cumPlan))}</small></td><td style="padding:8px 10px;border-bottom:1px solid #e7ebf0;white-space:nowrap">${r.behind>0?`<b style="color:#c8102e">Behind ${this.esc(fmt(r.behind))}${r.unit?' '+this.esc(r.unit):''}</b>`:(r.done>0?'<b style="color:#167846">Actual entered</b>':'<b style="color:#167846">On track</b>')}</td></tr>`).join('')).join('')}</tbody></table></div>`;};
    ...
    const areaCards=visibleCats.map(cat=>{const d=defs[cat],meta=areaMeta[cat],levels=(d&&d.levels)||[],all=[];levels.forEach(lv=>levelRows(cat,lv).forEach(r=>all.push({...r,cat})));const byAct={};all.forEach(r=>(byAct[r.aid]||(byAct[r.aid]={aid:r.aid,name:r.name,unit:r.unit,rows:[]})).rows.push(r));const acts=Object.values(byAct).sort((a,b)=>{const ai=actOrder.indexOf(a.aid),bi=actOrder.indexOf(b.aid),av=ai<0?999:ai,bv=bi<0?999:bi;return av-bv||String(a.name).localeCompare(String(b.name));});const actBlocks=acts.map(a=>{const mp=a.rows.reduce((n,r)=>n+r.plan,0),md=a.rows.reduce((n,r)=>n+r.done,0),owe=a.rows.reduce((n,r)=>n+r.behind,0),completed=a.rows.filter(r=>r.done>0),progress=a.rows.filter(r=>r.done<=0),empty=msg=>`<div style="padding:14px;text-align:center;color:#98a2b3;background:#fff;font-size:10px">${msg}</div>`;return `<section style="border:1px solid #d9e0e8;border-radius:8px;overflow:hidden;background:#fff;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;background:#e8edf3;border-bottom:1px solid #d9e0e8"><b style="font-size:15px;color:#202938">${this.esc(a.name)}</b><span style="font-size:9.5px;color:#667085;font-weight:700">${a.rows.length} zones · Plan ${this.esc(fmt(mp))} · Actual ${this.esc(fmt(md))}${owe>0?` · <b style="color:#c8102e">Behind ${this.esc(fmt(owe))}</b>`:''}</span></div><div style="padding:7px 10px;background:#dff3e7;color:#167846;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.35px">✓ Completed work this month · ${completed.length}</div>${completed.length?tableHtml(completed):empty('No actual quantity entered this month.')}<div style="padding:7px 10px;background:#ffead7;color:#a74d09;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.35px;border-top:1px solid #d9e0e8">● In progress / outstanding · ${progress.length}</div>${progress.length?tableHtml(progress):empty('No remaining rows without actual entry.')}</section>`;}).join('')||'<div style="padding:28px;text-align:center;color:#98a2b3">No activity data for this month.</div>';return `<article style="min-width:0;border:1px solid #d9e0e8;background:#fff;border-radius:10px;box-shadow:0 3px 14px rgba(24,39,75,.09);overflow:hidden;break-inside:avoid"><div style="display:flex;align-items:center;justify-content:space-between;padding:13px 17px;background:${meta.color};color:#fff"><div style="font-size:18px;font-weight:900">${this.esc(meta.name)}</div><div style="font-size:12px;font-weight:900;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.45);border-radius:20px;padding:4px 11px">${this.esc(mon)}</div></div><div style="display:grid;grid-template-columns:1fr;gap:15px;padding:16px">${actBlocks}</div></article>`;}).join('');
    ...
    ov.innerHTML=`<div style="max-width:1500px;margin:0 auto"><div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:14px"><div style="border-left:6px solid #6d1327;padding-left:14px;flex:1"><div style="font-size:23px;font-weight:800;line-height:1.2"><span style="color:#161616">P1 Waterfront</span> <span style="color:#6d1327">| Combined Construction Schedule</span></div><div style="font-size:11px;color:var(--dim);margin-top:5px">${this.esc(mon)} · Activity first · then Level and Zone detail</div></div><button class="hbtn" id="laClose">Close ✕</button></div><div class="seg" id="rptViewSeg" style="margin:0 0 11px"><button data-view="area">Area Report</button><button data-view="combined" class="on">Combined Schedule</button></div><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:10px"><div class="seg" id="rptCombinedAreas" style="margin:0">${areaBtns}</div><div style="display:flex;align-items:center;gap:6px;padding:5px 7px;background:var(--panel2);border:1px solid var(--line);border-radius:9px"><span style="font-size:9px;font-weight:900;color:var(--dim)">EXPORT LEVEL PNG</span><select id="rptExportArea" style="padding:5px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt)">${exportAreaOpts}</select><select id="rptExportLevel" style="padding:5px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--txt)">${exportLevelOpts}</select><button class="hbtn primary" id="rptExportPng" style="padding:5px 10px">Export image</button></div></div><div class="seg" id="rptCombinedMonths" style="display:flex;overflow-x:auto;gap:5px;margin:0 0 16px;padding:2px 1px 6px">${monthBtns}</div><div style="background:#f4f6f9;border:1px solid #dfe4eb;border-radius:12px;padding:18px;display:grid;grid-template-columns:1fr;gap:20px;margin-bottom:28px">${areaCards}</div></div>`;
  ...
  openLookAhead(){
    ...
    const makeRows=(reportRows)=>reportRows.map(r=>{ const liveA=r.actual||this._catchupActual(r.levels,r.aid,cat,r.filter),liveP=r.plan||this._catchupPlan(r.levels,r.aid,cat,r.filter),liveTotal=this._reportCommonTotal(r.levels,r.aid,cat,r.filter,liveA,liveP),ev=this._reportEditedValues(cat,r,liveA,liveP,liveTotal),A=ev.A,P=ev.P,den=A.total,tgt=den>0?Math.min(100,Math.round(P.planned/den*100)):0,unit=r.unit?(' '+r.unit):''; const actSub=`(${fmtN(A.done)}/${fmtN(den)}${unit})`,planSub=`(${fmtN(P.planned)}/${fmtN(den)}${unit} planned to date)`,by=P.end?this._fmtDShort(P.end):'—',inp='width:70px;padding:5px 6px;border:1px solid #cbaeb4;border-radius:5px;text-align:center;font:700 12px Segoe UI;background:#fff;color:#2b1114';
      ...
      const planEdit=editing?`<div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap;margin-top:6px"><label style="font-size:9px">Catch-Up<br><input class="rpt-edit" data-field="planned" type="number" min="0" step="1" value="${fmtN(P.planned)}" data-live="${fmtN(liveP.planned)}" style="${inp}"></label><label style="font-size:9px">Level total<br><input class="rpt-edit" data-field="total" type="number" min="0" step="1" value="${fmtN(den)}" data-live="${fmtN(liveTotal)}" style="${inp}"></label></div>`:'';
      const actEdit=editing?`<div style="margin-top:6px"><label style="font-size:9px">Actual done<br><input class="rpt-edit" data-field="done" type="number" min="0" step="1" value="${fmtN(A.done)}" data-live="${fmtN(liveA.done)}" style="${inp}"></label></div>`:'';
      return `<tr data-rkey="${this.esc(ev.key)}">`
        +`<td style="background:#6d1327;color:#fff;font-weight:800;padding:11px 9px;font-size:11.5px;vertical-align:middle;width:27%">${this.esc(r.a)}${this._reportCmtBtn(rc)}${editing?`<button type="button" class="rpt-live" style="display:block;margin-top:8px;padding:3px 7px;border:1px solid rgba(255,255,255,.65);border-radius:5px;background:${ev.has?'#fff':'transparent'};color:${ev.has?'#6d1327':'#fff'};font-size:8.5px;cursor:pointer">↺ Use live</button>`:''}</td>`
    ...
    const tables=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(420px,1fr));gap:18px;align-items:start">${def.levels.map(makeTable).join('')}</div>`;
  ...
  elRows(lv,z,type,items,metaFn,empty,critFn){if(!items||!items.length)return `<div class="empty">${empty}</div>`;
    return items.map(x=>{const id=(typeof x==='string')?x:x.id;const key=this.ekey(lv,z,type,id);const crit=critFn&&critFn(x);
      ...
      const _isPour=(type==='core'||type==='lift'||type==='stair'),_zmk=z.mk||z.lid,_pv=_isPour?this.elemPourPct(lv,_zmk,'ls',type,id):null,_canPour=_isPour&&(this.rwsIsAdmin()||this.rwsScopeOk(lv,_zmk));
      const _pourCtl=!_isPour?'':(_canPour?`<label class="elem-pour" title="Three-pour progress for this individual item"><span>Pour</span><input type="number" min="0" max="100" step="1" class="elem-pour-in" data-a="ls" data-type="${this.esc(type)}" data-id="${this.esc(id)}" value="${_pv==null?'':_pv}" placeholder="%"><b>%</b></label>`:(_pv==null?'':`<span class="elem-pour-read">Pour ${_pv}%</span>`));
